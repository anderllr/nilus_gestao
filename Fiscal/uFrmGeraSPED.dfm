object FrmSPED: TFrmSPED
  Left = 0
  Top = 0
  Hint = 
    'Sistema Integrado de Informa'#231#245'es sobre Opera'#231#245'es Interestaduais ' +
    'com Mercadorias e Servi'#231'os'
  BorderIcons = [biSystemMenu]
  Caption = 'Gera'#231#227'o do SPED Fiscal'
  ClientHeight = 522
  ClientWidth = 719
  Color = 15201011
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  ShowHint = True
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Bevel1: TBevel
    Left = 0
    Top = 484
    Width = 719
    Height = 4
    Align = alBottom
    Shape = bsFrame
    Style = bsRaised
    ExplicitTop = 194
    ExplicitWidth = 670
  end
  object Bevel3: TBevel
    Left = 0
    Top = 0
    Width = 719
    Height = 28
    Align = alTop
    ExplicitLeft = -8
    ExplicitTop = -6
    ExplicitWidth = 911
  end
  object Bevel5: TBevel
    Left = 0
    Top = 28
    Width = 719
    Height = 3
    Align = alTop
    Shape = bsTopLine
    Style = bsRaised
    ExplicitTop = 27
    ExplicitWidth = 691
  end
  object CLabel11: TCLabel
    Left = 520
    Top = 11
    Width = 18
    Height = 13
    Alignment = taRightJustify
    Caption = 'I.E.'
    Transparent = True
  end
  object CLabel2: TCLabel
    Left = 12
    Top = 11
    Width = 41
    Height = 13
    Alignment = taRightJustify
    Caption = 'Empresa'
  end
  object PanBotoes: TCPanelGradient
    Left = 0
    Top = 488
    Width = 719
    Height = 34
    Align = alBottom
    BevelOuter = bvNone
    Caption = 'PanBotoes'
    TabOrder = 1
    CaptionNo = 0
    HintNo = 0
    BackgroundGradientColor = 13099746
    BackgroundGradientDirection = gdVertical
    TextColor = clBlack
    GradientColor = clBlack
    RegularButtonColor = clBlack
    SelectedButtonColor = clBlack
    ButtonOptions = []
    object BtnPesquisar: TCBitBtn
      Left = 6
      Top = 132
      Width = 75
      Height = 25
      Caption = '&Pesquisar'
      DoubleBuffered = True
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00F7F7F700B5B5B500F7F7F700FF00FF00}
      NumGlyphs = 2
      ParentDoubleBuffered = False
      TabOrder = 2
      Visible = False
    end
    object BtnGerar: TCBitBtn
      Left = 12
      Top = 6
      Width = 75
      Height = 25
      Action = actGerar
      Caption = '&Gerar'
      DoubleBuffered = True
      Glyph.Data = {
        
        E5FFFBFBFBFFFEFEFEFFFF00FF00FF00FF00FF00FF00FF00FF00}
      ParentDoubleBuffered = False
      TabOrder = 0
    end
    object BtnFechar: TCBitBtn
      Left = 551
      Top = 6
      Width = 75
      Height = 25
      Action = actFechar
      Caption = '&Fechar'
      DoubleBuffered = True
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentDoubleBuffered = False
      TabOrder = 1
    end
  end
  object CGroupBox1: TCGroupBox
    Left = 8
    Top = 34
    Width = 668
    Height = 83
    Caption = 'Param'#234'tros'
    TabOrder = 0
    object CLabel3: TCLabel
      Left = 14
      Top = 24
      Width = 100
      Height = 13
      Alignment = taRightJustify
      Caption = 'Informe o M'#234's e Ano'
    end
    object CLabel4: TCLabel
      Left = 14
      Top = 51
      Width = 112
      Height = 13
      Caption = 'Caminho do arquivo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object CLabel1: TCLabel
      Left = 206
      Top = 24
      Width = 175
      Height = 13
      Alignment = taRightJustify
      Caption = '(Ex.: 08/2007 para Agosto de 2007)'
    end
    object EDCaminho: TCEdit
      Left = 132
      Top = 48
      Width = 333
      Height = 19
      TabStop = False
      BorderStyle = bsNone
      BevelKind = bkTile
      Color = 12506838
      ReadOnly = True
      TabOrder = 1
      DataType = ftUnknown
      KeyMode = kmNormal
      Decimals = 0
    end
    object BtnCaminho: TCBitBtn
      Left = 471
      Top = 48
      Width = 21
      Height = 19
      DoubleBuffered = True
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentDoubleBuffered = False
      TabOrder = 2
      OnClick = BtnCaminhoClick
    end
    object DBEData: TCDBEdit
      Left = 120
      Top = 21
      Width = 80
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'mesAno'
      DataSource = dsPadrao
      TabOrder = 0
      OnEnter = DBEDataEnter
      OnExit = DBEDataExit
      DecimalControl = True
      KeyMode = kmNormal
    end
  end
  object DBEdtInicial: TCDBEdit
    Left = 597
    Top = 181
    Width = 73
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'dtInicial'
    DataSource = dsPadrao
    TabOrder = 2
    Visible = False
    DecimalControl = True
    KeyMode = kmNormal
  end
  object DBEdtFinal: TCDBEdit
    Left = 597
    Top = 156
    Width = 73
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'dtFinal'
    DataSource = dsPadrao
    TabOrder = 3
    Visible = False
    DecimalControl = True
    KeyMode = kmNormal
  end
  object LookidInscEstadual: TCLookUp
    Left = 544
    Top = 8
    Width = 126
    Height = 19
    Alignment = taLeftJustify
    BevelInner = bvSpace
    BevelOuter = bvLowered
    Color = 14149350
    ParentBackground = False
    TabOrder = 4
    Key.Strings = (
      'idEmpresa'
      'idCadEmpresa')
    LookUpKey.Strings = (
      'idEmpresa'
      'idCadEmpresa')
    ClientDataSet = cdsLookUp
    DataSource = dsPadrao
    ValidaCampoObrigatorio = True
    CampoObrigatorio = True
    LookUpTable = 'vCadEmpresaFiscal'
    ReturnField = 'idInscEstadual'
  end
  object DBEidEmpresa: TCDBEdit
    Left = 59
    Top = 8
    Width = 46
    Height = 19
    TabStop = False
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'idEmpresa'
    DataSource = dsPadrao
    TabOrder = 5
    OnKeyDown = DBEidEmpresaKeyDown
    DecimalControl = True
    KeyMode = kmNormal
  end
  object DBEidCadEmpresa: TCDBEdit
    Left = 111
    Top = 8
    Width = 35
    Height = 19
    TabStop = False
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'idCadEmpresa'
    DataSource = dsPadrao
    TabOrder = 6
    OnExit = DBEidCadEmpresaExit
    OnKeyDown = DBEidEmpresaKeyDown
    DecimalControl = True
    KeyMode = kmNormal
  end
  object LookdescCadEmpresa: TCLookUp
    Left = 152
    Top = 8
    Width = 202
    Height = 19
    Alignment = taLeftJustify
    BevelInner = bvSpace
    BevelOuter = bvLowered
    Color = 14149350
    ParentBackground = False
    TabOrder = 7
    Key.Strings = (
      'idEmpresa'
      'idCadEmpresa')
    LookUpKey.Strings = (
      'idEmpresa'
      'idCadEmpresa')
    ClientDataSet = cdsLookUp
    DataSource = dsPadrao
    ValidaCampoObrigatorio = True
    CampoObrigatorio = True
    LookUpTable = 'vCadEmpresaFiscal'
    ReturnField = 'descCadEmpresa'
  end
  object LookdescAbreviada: TCLookUp
    Left = 360
    Top = 8
    Width = 154
    Height = 19
    Alignment = taLeftJustify
    BevelInner = bvSpace
    BevelOuter = bvLowered
    Color = 14149350
    ParentBackground = False
    TabOrder = 8
    Key.Strings = (
      'idEmpresa'
      'idCadEmpresa')
    LookUpKey.Strings = (
      'idEmpresa'
      'idCadEmpresa')
    ClientDataSet = cdsLookUp
    DataSource = dsPadrao
    ValidaCampoObrigatorio = True
    CampoObrigatorio = True
    LookUpTable = 'vCadEmpresaFiscal'
    ReturnField = 'descAbreviada'
  end
  object DBEpar: TCDBEdit
    Left = 641
    Top = 131
    Width = 29
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'par'
    DataSource = dsPadrao
    MaxLength = 10
    TabOrder = 9
    Visible = False
    DecimalControl = True
    KeyMode = kmUSUpper
  end
  object CBConcomitante: TCCheckBox
    Left = 551
    Top = 451
    Width = 119
    Height = 17
    Caption = 'Gerar Concomitante'
    TabOrder = 10
  end
  object CRCodFinalidade: TCRadioGroup
    Left = 2
    Top = 123
    Width = 185
    Height = 78
    Caption = 'C'#243'digo da Finalidade'
    Items.Strings = (
      'Remessa do arquivo original'
      'Remessa do arquivo substituto')
    TabOrder = 11
  end
  object CMTxt: TCMemo
    Left = 8
    Top = 379
    Width = 537
    Height = 46
    BevelKind = bkTile
    BorderStyle = bsNone
    Lines.Strings = (
      '')
    TabOrder = 12
    Visible = False
  end
  object CRIndMov: TCRadioGroup
    Left = 193
    Top = 123
    Width = 139
    Height = 78
    Caption = 'Inficador de Movimento'
    Items.Strings = (
      '0- Bloco com dados'
      '1- Bloco sem dados')
    TabOrder = 13
  end
  object CRcodModelo: TCRadioGroup
    Left = 338
    Top = 123
    Width = 176
    Height = 78
    Caption = 'Cod. do Modelo de Arrecada'#231#227'o'
    Enabled = False
    Items.Strings = (
      '0 - documento estadual de arrecada'#231#227'o'
      '1 '#8211' GNRE')
    TabOrder = 14
    Visible = False
  end
  object Acao: TActionList
    Images = imgIcones
    Left = 536
    Top = 40
    object actFechar: TAction
      Caption = '&Fechar'
      ImageIndex = 1
      OnExecute = actFecharExecute
    end
    object actGerar: TAction
      Caption = '&Gerar'
      ImageIndex = 2
      OnExecute = actGerarExecute
    end
  end
  object imgIcones: TImageList
    DrawingStyle = dsTransparent
    Left = 560
    Top = 40
    Bitmap = {
      
      000000000000}
  end
  object AbrePasta: TcxShellBrowserDialog
    Left = 496
    Top = 40
  end
  object dsPadrao: TDataSource
    DataSet = cdsPadrao
    Left = 624
    Top = 40
  end
  object cdsPadrao: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DmPadrao'
    Left = 592
    Top = 40
    object cdsPadraodtInicial: TDateTimeField
      FieldName = 'dtInicial'
    end
    object cdsPadraodtFinal: TDateTimeField
      FieldName = 'dtFinal'
    end
    object cdsPadraoidEmpresa: TIntegerField
      FieldName = 'idEmpresa'
    end
    object cdsPadraoidCadEmpresa: TIntegerField
      FieldName = 'idCadEmpresa'
    end
    object cdsPadraomesAno: TStringField
      FieldName = 'mesAno'
    end
    object cdsPadraopar: TStringField
      FieldName = 'par'
      Size = 1
    end
    object cdsPadraoidFinalidade: TSmallintField
      FieldName = 'idFinalidade'
    end
    object cdsPadraodescCadEmpresa: TStringField
      FieldName = 'descCadEmpresa'
      Size = 50
    end
    object cdsPadraocnpj: TStringField
      FieldName = 'cnpj'
      Size = 14
    end
    object cdsPadraocpf: TStringField
      FieldName = 'cpf'
      Size = 11
    end
    object cdsPadraouf: TStringField
      FieldName = 'uf'
      Size = 2
    end
    object cdsPadraoie: TStringField
      FieldName = 'ie'
      Size = 14
    end
    object cdsPadraoim: TStringField
      FieldName = 'im'
    end
    object cdsPadraosuframa: TStringField
      FieldName = 'suframa'
      Size = 9
    end
    object cdsPadraocodmun: TStringField
      FieldName = 'codmun'
      Size = 10
    end
    object cdsPadraodescFanstasia: TStringField
      FieldName = 'descAbreviada'
      Size = 50
    end
    object cdsPadraoEndereco: TStringField
      FieldName = 'endereco'
      Size = 50
    end
    object cdsPadraoComplemento: TStringField
      FieldName = 'complemento'
      Size = 30
    end
    object cdsPadraoBairro: TStringField
      FieldName = 'bairro'
      Size = 40
    end
    object cdsPadraoFone: TStringField
      FieldName = 'fone'
    end
    object cdsPadraoFax: TStringField
      FieldName = 'fax'
    end
    object cdsPadraoemail: TStringField
      FieldName = 'email'
      Size = 50
    end
    object cdsPadraonum: TStringField
      FieldName = 'numero'
      Size = 10
    end
    object cdsPadraocep: TStringField
      FieldName = 'cep'
      Size = 10
    end
    object cdsPadraosubsTributario: TStringField
      FieldName = 'subsTributario'
      Size = 1
    end
    object cdsPadraotpAtividade: TStringField
      FieldName = 'tpAtividade'
      Size = 40
    end
    object cdsPadraoperfilApresentacao: TStringField
      FieldName = 'perfilApresentacao'
    end
    object cdsPadraoidProdDeposito: TIntegerField
      FieldName = 'idProdDeposito'
    end
    object cdsPadraoidSafra: TIntegerField
      FieldName = 'idSafra'
    end
    object cdsPadraoidCultura: TIntegerField
      FieldName = 'idCultura'
    end
  end
  object cdsLookUp: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspLookUp'
    AfterInsert = cdsLookUpAfterInsert
    Left = 632
    Top = 80
  end
  object dspLookUp: TDataSetProvider
    DataSet = dmPadrao.sdsLookup
    Options = [poPropogateChanges, poAllowCommandText]
    Left = 600
    Top = 80
  end
  object ACBrSPEDFiscal1: TACBrSPEDFiscal
    Arquivo = 'SPEDFiscal.txt'
    Delimitador = '|'
    TrimString = True
    CurMascara = '#0.00'
    Left = 566
    Top = 80
  end
  object sdsEmpresaContabil: TSQLDataSet
    SchemaName = 'gesys'
    CommandText = 'SELECT * FROM vCadEmpresaContabilSPED'
    DbxCommandType = 'Dbx.SQL'
    MaxBlobSize = -1
    Params = <>
    SQLConnection = dmPadrao.dbConexao
    Left = 536
    Top = 124
  end
  object cdsEmpresaContabil: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspEmpresaContabil'
    Left = 600
    Top = 124
    object cdsEmpresaContabilidCadGeral: TIntegerField
      FieldName = 'idCadGeral'
      Required = True
    end
    object cdsEmpresaContabildescRepresentante: TStringField
      FieldName = 'descRepresentante'
      Size = 70
    end
    object cdsEmpresaContabilidCnpjCpfRepresentante: TStringField
      FieldName = 'idCnpjCpfRepresentante'
      Size = 18
    end
    object cdsEmpresaContabilcargoRepresentante: TStringField
      FieldName = 'cargoRepresentante'
      Size = 40
    end
    object cdsEmpresaContabildescContador: TStringField
      FieldName = 'descContador'
      Size = 70
    end
    object cdsEmpresaContabilidCnpjCpfContador: TStringField
      FieldName = 'idCnpjCpfContador'
      Size = 18
    end
    object cdsEmpresaContabilcrcContador: TStringField
      FieldName = 'crcContador'
      Size = 30
    end
    object cdsEmpresaContabilimgLogo: TBlobField
      FieldName = 'imgLogo'
      Size = 1
    end
  end
  object dspEmpresaContabil: TDataSetProvider
    DataSet = sdsEmpresaContabil
    Options = [poAllowMultiRecordUpdates, poAllowCommandText, poUseQuoteChar]
    Left = 568
    Top = 124
  end
end
