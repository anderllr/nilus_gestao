inherited FrmRelRelacContabil: TFrmRelRelacContabil
  Caption = 'Relacionamento Cont'#225'beis'
  ClientHeight = 329
  ClientWidth = 734
  ExplicitWidth = 750
  ExplicitHeight = 368
  PixelsPerInch = 96
  TextHeight = 13
  inherited Bevel3: TBevel
    Width = 734
    ExplicitWidth = 790
  end
  inherited Bevel1: TBevel
    Top = 291
    Width = 734
    ExplicitTop = 291
    ExplicitWidth = 790
  end
  inherited Bevel5: TBevel
    Width = 734
    ExplicitWidth = 790
  end
  object CLabel10: TCLabel [4]
    Left = 11
    Top = 86
    Width = 76
    Height = 13
    Alignment = taRightJustify
    Caption = 'Mov. Financeira'
    Transparent = True
  end
  object CLabel6: TCLabel [5]
    Left = 392
    Top = 86
    Width = 41
    Height = 13
    Alignment = taRightJustify
    Caption = 'Hist'#243'rico'
    Transparent = True
  end
  object CLabel7: TCLabel [6]
    Left = 402
    Top = 108
    Width = 31
    Height = 13
    Alignment = taRightJustify
    Caption = 'D'#233'bito'
    Transparent = True
  end
  object CLabel8: TCLabel [7]
    Left = 398
    Top = 130
    Width = 35
    Height = 13
    Alignment = taRightJustify
    Caption = 'Cr'#233'dito'
    Transparent = True
  end
  object CLabel9: TCLabel [8]
    Left = 39
    Top = 130
    Width = 48
    Height = 13
    Alignment = taRightJustify
    Caption = 'Resultado'
    Transparent = True
  end
  object CLabel12: TCLabel [9]
    Left = 43
    Top = 107
    Width = 44
    Height = 13
    Alignment = taRightJustify
    Caption = 'Gerencial'
    Transparent = True
  end
  inherited PanBotoes: TCPanelGradient
    Top = 295
    Width = 734
    TabOrder = 16
    ExplicitTop = 295
    ExplicitWidth = 734
    inherited BtnFechar: TCBitBtn
      Left = 654
      ExplicitLeft = 654
    end
  end
  inherited DBEidEmpresa: TCDBEdit
    TabOrder = 0
  end
  inherited DBEidCadEmpresa: TCDBEdit
    TabOrder = 1
  end
  object DBEidMovFinanceira: TCDBEdit [15]
    Left = 93
    Top = 83
    Width = 48
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'idMovFinanceira'
    DataSource = dsPadrao
    TabOrder = 5
    Find = FindMovFinanceira
    AcaoCad = frmPrincipal.actCadMovFinanceira
    DecimalControl = True
    KeyMode = kmNormal
  end
  object DBEidHistorico: TCDBEdit [16]
    Left = 439
    Top = 83
    Width = 48
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'idContHistorico'
    DataSource = dsPadrao
    TabOrder = 7
    Find = FindContHistorico
    DecimalControl = True
    KeyMode = kmNormal
  end
  object LookHistorico: TCLookUp [17]
    Left = 493
    Top = 83
    Width = 229
    Height = 19
    Alignment = taLeftJustify
    BevelInner = bvSpace
    BevelOuter = bvLowered
    Color = 14149350
    ParentBackground = False
    TabOrder = 8
    Key.Strings = (
      'idContHistorico')
    LookUpKey.Strings = (
      'idContHistorico')
    ClientDataSet = cdsLookUp
    DataSource = dsPadrao
    ValidaCampoObrigatorio = False
    CampoObrigatorio = True
    LookUpTable = 'ContHistorico'
    ReturnField = 'descContHistorico'
  end
  object DBEcdPlanoContaDeb: TCDBEdit [18]
    Left = 439
    Top = 105
    Width = 48
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'cdPlanoContaDeb'
    DataSource = dsPadrao
    TabOrder = 11
    Find = FindPlanoContaDeb
    DecimalControl = True
    KeyMode = kmNormal
  end
  object DBEcdPlanoContaCre: TCDBEdit [19]
    Left = 439
    Top = 127
    Width = 48
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'cdPlanoContaCre'
    DataSource = dsPadrao
    TabOrder = 14
    Find = FindPlanoContaCre
    DecimalControl = True
    KeyMode = kmNormal
  end
  object LookGerencial: TCLookUp [20]
    Left = 144
    Top = 105
    Width = 234
    Height = 19
    Alignment = taLeftJustify
    BevelInner = bvSpace
    BevelOuter = bvLowered
    Color = 14149350
    ParentBackground = False
    TabOrder = 10
    Key.Strings = (
      'idGerencial')
    LookUpKey.Strings = (
      'idGerencial')
    AlternateSQL.Strings = (
      
        'SELECT DISTINCT descGerencial FROM vGerencial WHERE StGerencial ' +
        '= '#39'ATIVO'#39' AND tpGerencial = '#39'SAIDAS'#39' AND idEmpresa = [DBEidEmpre' +
        'sa] AND idGerencial = ?')
    ClientDataSet = cdsLookUp
    DataSource = dsPadrao
    ValidaCampoObrigatorio = False
    CampoObrigatorio = True
    LookUpTable = 'vGerencial'
    ReturnField = 'descGerencial'
  end
  object LookResultado: TCLookUp [21]
    Left = 144
    Top = 127
    Width = 234
    Height = 19
    Alignment = taLeftJustify
    BevelInner = bvSpace
    BevelOuter = bvLowered
    Color = 14149350
    ParentBackground = False
    TabOrder = 13
    Key.Strings = (
      'idResultado')
    LookUpKey.Strings = (
      'idResultado')
    AlternateSQL.Strings = (
      
        'SELECT descResultado FROM Resultado WHERE idEmpresa = [DBEidEmpr' +
        'esa] AND idResultado = ?')
    ClientDataSet = cdsLookUp
    DataSource = dsPadrao
    ValidaCampoObrigatorio = False
    CampoObrigatorio = True
    LookUpTable = 'Resultado'
    ReturnField = 'descResultado'
  end
  object DBEidResultado: TCDBEdit [22]
    Left = 93
    Top = 127
    Width = 48
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'idResultado'
    DataSource = dsPadrao
    TabOrder = 12
    Find = FindResultado
    DecimalControl = True
    KeyMode = kmNormal
  end
  object DBEidGerencial: TCDBEdit [23]
    Left = 93
    Top = 105
    Width = 48
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'idGerencial'
    DataSource = dsPadrao
    TabOrder = 9
    Find = FindGerencial
    DecimalControl = True
    KeyMode = kmNormal
  end
  object LookMovFinanceira: TCLookUp [24]
    Left = 144
    Top = 83
    Width = 234
    Height = 19
    Alignment = taLeftJustify
    BevelInner = bvSpace
    BevelOuter = bvLowered
    Color = 14149350
    ParentBackground = False
    TabOrder = 6
    Key.Strings = (
      'idMovFinanceira')
    LookUpKey.Strings = (
      'idMovFinanceira')
    ClientDataSet = cdsLookUp
    DataSource = dsPadrao
    ValidaCampoObrigatorio = True
    CampoObrigatorio = True
    LookUpTable = 'MovFinanceira'
    ReturnField = 'descMovFinanceira'
  end
  object DBRGtpPlanoConta: TCDBRadioGroup [25]
    Left = 8
    Top = 32
    Width = 703
    Height = 45
    Caption = 'Origem'
    Columns = 2
    DataField = 'tpPlanoConta'
    DataSource = dsPadrao
    DragCursor = crArrow
    Items.Strings = (
      '&Fiscal'
      '&Gerencial')
    TabOrder = 4
    Values.Strings = (
      'F'
      'G')
    OnChange = DBRGtpPlanoContaChange
  end
  object EDtpPlanoConta: TCEdit [26]
    Left = 706
    Top = 167
    Width = 57
    Height = 19
    BorderStyle = bsNone
    BevelKind = bkTile
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 15
    Text = 'F'
    Visible = False
    DataType = ftUnknown
    KeyMode = kmNormal
    Decimals = 0
  end
  object LookdescPlanoContaDeb: TCLookUp [27]
    Left = 493
    Top = 103
    Width = 229
    Height = 19
    Alignment = taLeftJustify
    BevelInner = bvSpace
    BevelOuter = bvLowered
    Color = 14149350
    ParentBackground = False
    TabOrder = 17
    Key.Strings = (
      'cdPlanoContaDeb')
    LookUpKey.Strings = (
      'cdPlanoConta')
    ClientDataSet = cdsLookUp
    DataSource = dsPadrao
    ValidaCampoObrigatorio = False
    CampoObrigatorio = True
    LookUpTable = 'vPlanoConta'
    ReturnField = 'descPlanoConta'
  end
  object LookdescPlanoContaCre: TCLookUp [28]
    Left = 493
    Top = 125
    Width = 229
    Height = 19
    Alignment = taLeftJustify
    BevelInner = bvSpace
    BevelOuter = bvLowered
    Color = 14149350
    ParentBackground = False
    TabOrder = 18
    Key.Strings = (
      'cdPlanoContaCre')
    LookUpKey.Strings = (
      'cdPlanoConta')
    ClientDataSet = cdsLookUp
    DataSource = dsPadrao
    ValidaCampoObrigatorio = False
    CampoObrigatorio = True
    LookUpTable = 'vPlanoConta'
    ReturnField = 'descPlanoConta'
  end
  inherited imgIcones: TImageList
    Bitmap = {
      
      000000000000}
  end
  inherited cdsRelatorio: TClientDataSet
    CommandText = 
      'SELECT R.*, CASE WHEN R.cdPlanoContaCre <> 0 THEN P.descPlanoCon' +
      'ta END AS descPlanoContaCre, CASE WHEN R.cdPlanoContaDeb <> 0 TH' +
      'EN P.descPlanoConta END AS descPlanoContaDeb FROM vPesRelaciona ' +
      'R LEFT JOIN PlanoConta P ON R.cdPlanoContaCre = P.cdPlanoConta O' +
      'R R.cdPlanoContaDeb = P.cdPlanoConta'
  end
  inherited frxDataset: TfrxDBDataset
    FieldAliases.Strings = (
      'idGerencial=idGerencial'
      'idMovFinanceira=idMovFinanceira'
      'idEmpresa=idEmpresa'
      'descMovFinanceira=descMovFinanceira'
      'idResultado=idResultado'
      'idContHistorico=idContHistorico'
      'descContHistorico=descContHistorico'
      'cdPlanoContaDeb=cdPlanoContaDeb'
      'cdPlanoContaCre=cdPlanoContaCre'
      'descGerencial=descGerencial'
      'descResultado=descResultado'
      'tpPlanoConta=tpPlanoConta'
      'classificacao=classificacao'
      'descPlanoContaCre=descPlanoContaCre'
      'descPlanoContaDeb=descPlanoContaDeb')
    Left = 477
  end
  inherited cdsPadrao: TClientDataSet
    AfterInsert = cdsPadraoAfterInsert
    object cdsPadraoidEmpresa: TIntegerField
      FieldName = 'idEmpresa'
    end
    object cdsPadraoidMovFinanceira: TIntegerField
      FieldName = 'idMovFinanceira'
    end
    object cdsPadraoidGerencial: TIntegerField
      FieldName = 'idGerencial'
    end
    object cdsPadraoidResultado: TIntegerField
      FieldName = 'idResultado'
    end
    object cdsPadraocdPlanoContaDeb: TIntegerField
      FieldName = 'cdPlanoContaDeb'
    end
    object cdsPadraotpPlanoConta: TStringField
      FieldName = 'tpPlanoConta'
    end
    object cdsPadraocdPlanoContaCre: TIntegerField
      FieldName = 'cdPlanoContaCre'
    end
    object cdsPadraoidContHistorico: TIntegerField
      FieldName = 'idContHistorico'
    end
  end
  inherited frxHTMLExport1: TfrxHTMLExport
    Left = 407
    Top = 119
  end
  inherited frxTIFFExport1: TfrxTIFFExport
    Left = 445
    Top = 89
  end
  inherited frxRTFExport1: TfrxRTFExport
    Left = 477
  end
  inherited frxJPEGExport1: TfrxJPEGExport
    Left = 407
    Top = 87
  end
  object frxRel: TfrxReport
    Version = '5.1.5'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.MDIChild = True
    PreviewOptions.Modal = False
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.Author = 'Gesys Sistemas de Informa'#231#227'o'
    ReportOptions.CreateDate = 39263.419322395800000000
    ReportOptions.LastChange = 41022.675202488400000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      '    '
      'end.')
    Left = 448
    Top = 48
    Datasets = <
      item
        DataSet = frxDataset
        DataSetName = 'frxDataset'
      end
      item
        DataSet = dmPadrao.frxEmpresaContabil
        DataSetName = 'frxEmpresaContabil'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 256
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        Height = 143.093005800000000000
        Top = 18.897650000000000000
        Width = 793.701300000000000000
        object Line2: TfrxLineView
          Left = 30.236240000000000000
          Top = 137.354360000000000000
          Width = 733.228820000000000000
          Color = clBlack
          Frame.Color = clGray
          Frame.ShadowColor = clGray
          Diagonal = True
        end
        object lblEmpresa: TfrxMemoView
          Left = 188.858380000000000000
          Top = 15.322766300000000000
          Width = 419.527830000000000000
          Height = 37.795300000000000000
          AutoWidth = True
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -24
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxEmpresaContabil."descEmpresa"]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Picture1: TfrxPictureView
          Left = 26.456710000000000000
          Top = 15.204700000000000000
          Width = 155.000000000000000000
          Height = 62.000000000000000000
          DataField = 'imgLogo'
          DataSet = dmPadrao.frxEmpresaContabil
          DataSetName = 'frxEmpresaContabil'
          HightQuality = False
          Transparent = False
          TransparentColor = clWhite
        end
        object Memo7: TfrxMemoView
          Left = 629.401980000000000000
          Top = 15.118120000000000000
          Width = 143.622140000000000000
          Height = 117.165430000000000000
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'RELACIONAMENTOS CONTABEIS')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo30: TfrxMemoView
          Left = 188.976500000000000000
          Top = 53.118120000000010000
          Width = 419.527830000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            
              'CNPJ/CPF: [(<frxEmpresaContabil."idCnpjCpf">)]  IE: [(<frxEmpres' +
              'aContabil."idInscEstadual">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo31: TfrxMemoView
          Left = 188.976500000000000000
          Top = 72.795300000000000000
          Width = 419.527830000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            
              '[(<frxEmpresaContabil."endereco">)] - [(<frxEmpresaContabil."com' +
              'plemento">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo34: TfrxMemoView
          Left = 188.976500000000000000
          Top = 93.354360000000000000
          Width = 419.527830000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            
              'CEP: [(<frxEmpresaContabil."cep">)]  -  [(<frxEmpresaContabil."d' +
              'escCidade">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo35: TfrxMemoView
          Left = 188.976500000000000000
          Top = 113.031540000000000000
          Width = 419.527830000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'Fone: [(<frxEmpresaContabil."fones">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Line4: TfrxLineView
          Left = 627.401980000000000000
          Top = 15.204700000000000000
          Height = 120.944960000000000000
          Color = clBlack
          Frame.Color = clGray
          Frame.ShadowColor = clGray
          Frame.Typ = [ftLeft]
        end
      end
      object GHRelatorio: TfrxGroupHeader
        FillType = ftBrush
        Height = 40.015770000000000000
        Top = 222.992270000000000000
        Width = 793.701300000000000000
        Condition = 'frxDataset."idGerencial"'
        ExpandDrillDown = True
        ReprintOnNewPage = True
        object Shape1: TfrxShapeView
          Left = 22.456692910000000000
          Top = 21.133890000000010000
          Width = 752.125996460000000000
          Height = 15.118120000000000000
          Fill.BackColor = 15724527
          Frame.Color = 15790320
        end
        object DBMTitdescGrupo: TfrxMemoView
          Left = 27.196847950000000000
          Top = 21.133890000000010000
          Width = 85.275510000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Mov. Financeira')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo3: TfrxMemoView
          Left = 195.943451240000000000
          Top = 21.023622050000000000
          Width = 55.039270000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Resultado')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo5: TfrxMemoView
          Left = 362.447471240000000000
          Top = 21.023622050000000000
          Width = 55.039270000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Historico')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo1: TfrxMemoView
          Left = 26.456710000000000000
          Width = 51.259740000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Gerencial:')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo2: TfrxMemoView
          Left = 481.864911240000000000
          Top = 20.992125980000000000
          Width = 36.141620000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Debito')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo9: TfrxMemoView
          Left = 637.471281240000000000
          Top = 21.125984249999990000
          Width = 36.141620000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Credito')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo11: TfrxMemoView
          Left = 532.203501240000000000
          Top = 21.125984249999990000
          Width = 81.495980000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Nome da Conta')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo14: TfrxMemoView
          Left = 185.535560000000000000
          Width = 228.897650000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            '[(<frxDataset."descGerencial">)]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo16: TfrxMemoView
          Left = 682.046111240000000000
          Top = 21.125984249999990000
          Width = 81.495980000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Nome da Conta')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo19: TfrxMemoView
          Left = 83.149660000000000000
          Width = 13.464440000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            '[(<frxDataset."idGerencial">)]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo20: TfrxMemoView
          Left = 106.944960000000000000
          Width = 73.936920000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            '[(<frxDataset."classificacao">)]')
          ParentFont = False
          VAlign = vaBottom
        end
      end
      object Band4: TfrxMasterData
        FillType = ftBrush
        Height = 18.141754250000000000
        Top = 287.244280000000000000
        Width = 793.701300000000000000
        Columns = 1
        ColumnWidth = 200.000000000000000000
        ColumnGap = 20.000000000000000000
        DataSet = frxDataset
        DataSetName = 'frxDataset'
        RowCount = 0
        object DBMdescGrupo: TfrxMemoView
          Left = 25.820496710000000000
          Width = 164.787384490000000000
          Height = 15.118110240000000000
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descMovFinanceira">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo4: TfrxMemoView
          Left = 194.567100000000000000
          Width = 161.007864250000000000
          Height = 15.118110240000000000
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descResultado">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo6: TfrxMemoView
          Left = 357.071120000000000000
          Width = 126.992094250000000000
          Height = 15.118110240000000000
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descContHistorico">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo8: TfrxMemoView
          Left = 482.488560000000000000
          Width = 47.621964250000000000
          Height = 15.118110240000000000
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HideZeros = True
          Memo.UTF8W = (
            '[(<frxDataset."cdPlanoContaDeb">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo10: TfrxMemoView
          Left = 636.094930000000000000
          Width = 47.621964250000000000
          Height = 15.118110240000000000
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HideZeros = True
          Memo.UTF8W = (
            '[(<frxDataset."cdPlanoContaCre">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo12: TfrxMemoView
          Left = 532.827150000000000000
          Width = 115.653504250000000000
          Height = 15.118110240000000000
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descPlanoContaDeb">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo18: TfrxMemoView
          Left = 682.669760000000000000
          Top = 0.110255750000021600
          Width = 115.653504250000000000
          Height = 15.118110240000000000
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descPlanoContaCre">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
      end
      object GroupFooter1: TfrxGroupFooter
        FillType = ftBrush
        Height = 24.456710000000000000
        Top = 328.819110000000000000
        Width = 793.701300000000000000
      end
      object PageFooter1: TfrxPageFooter
        FillType = ftBrush
        Height = 66.811070000000000000
        Top = 415.748300000000000000
        Width = 793.701300000000000000
        object Memo13: TfrxMemoView
          Left = 316.157700000000000000
          Top = 7.181102362204740000
          Width = 117.165430000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Nilus Report ')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo15: TfrxMemoView
          Left = 26.456692910000000000
          Top = 7.181102362204740000
          Width = 275.905690000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Nilus Solu'#231#245'es para o Agroneg'#243'cio Ltda.')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo17: TfrxMemoView
          Left = 618.583180000000000000
          Top = 7.181102362204740000
          Width = 132.283550000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[(<Page>)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Line1: TfrxLineView
          Left = 26.456692910000000000
          Top = 3.401359999999898000
          Width = 733.228346460000000000
          Color = clBlack
          Frame.Typ = [ftTop]
          Frame.Width = 0.100000000000000000
        end
        object Memo58: TfrxMemoView
          Left = 454.000000000000000000
          Top = 7.181102359999898000
          Width = 117.165430000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<Date>)] - [(<Time>)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
      end
    end
  end
  object FindResultado: TCFind
    SelectClause.Strings = (
      'SELECT idResultado, descResultado, StResultado FROM Resultado')
    JoinClause.Strings = (
      'WHERE stResultado = '#39'ATIVO'#39' AND idEmpresa = [DBEidEmpresa]')
    OrderByClause.Strings = (
      'ORDER BY descResultado')
    FindField = 'descResultado'
    SQLConnection = dmPadrao.dbConexao
    ValueField = 'idResultado'
    CarregaDados = True
    CollumNames.Strings = (
      'C'#243'digo'
      'Descri'#231#227'o'
      'Status')
    TypeFind = fFindNormal
    Left = 216
    Top = 186
  end
  object FindContHistorico: TCFind
    SelectClause.Strings = (
      
        'SELECT idContHistorico, descContHistorico, stContHistorico FROM ' +
        'ContHistorico')
    JoinClause.Strings = (
      'WHERE stContHistorico = '#39'ATIVO'#39)
    OrderByClause.Strings = (
      'ORDER BY descContHistorico')
    FindField = 'descContHistorico'
    SQLConnection = dmPadrao.dbConexao
    ValueField = 'idContHistorico'
    CarregaDados = True
    CollumNames.Strings = (
      'C'#243'digo'
      'Descri'#231#227'o'
      'Status')
    TypeFind = fFindNormal
    Left = 120
    Top = 186
  end
  object FindGerencial: TCFind
    SelectClause.Strings = (
      
        'SELECT DISTINCT idGerencial, classificacao, descGerencial FROM v' +
        'Gerencial')
    JoinClause.Strings = (
      'WHERE StGerencial = '#39'ATIVO'#39' AND idEmpresa = [DBEidEmpresa]')
    OrderByClause.Strings = (
      'ORDER BY descGerencial')
    FindField = 'descGerencial'
    SQLConnection = dmPadrao.dbConexao
    ValueField = 'idGerencial'
    CarregaDados = True
    CollumNames.Strings = (
      'C'#243'digo'
      'Classifica'#231#227'o'
      'Descri'#231#227'o')
    TypeFind = fFindNormal
    Left = 152
    Top = 186
  end
  object FindMovFinanceira: TCFind
    SelectClause.Strings = (
      'SELECT  idMovFinanceira, descMovFinanceira FROM MovFinanceira')
    JoinClause.Strings = (
      'WHERE StMovFinanceira = '#39'ATIVO'#39)
    OrderByClause.Strings = (
      'ORDER BY descMovFinanceira')
    FindField = 'descMovFinanceira'
    SQLConnection = dmPadrao.dbConexao
    ValueField = 'idMovFinanceira'
    CarregaDados = True
    CollumNames.Strings = (
      'C'#243'digo'
      'Descri'#231#227'o')
    TypeFind = fFindNormal
    Left = 184
    Top = 186
  end
  object FindContRelaciona: TCFind
    SelectClause.Strings = (
      
        'SELECT DISTINCT cr.idEmpresa,emp.descCadEmpresa FROM ContRelacio' +
        'na cr, vCadEmpresaContabil emp')
    JoinClause.Strings = (
      'WHERE cr.idEmpresa = emp.idEmpresa')
    OrderByClause.Strings = (
      'ORDER BY emp.descCadEmpresa')
    FindField = 'idEmpresa'
    SQLConnection = dmPadrao.dbConexao
    ValueField = 'idEmpresa'
    CarregaDados = True
    CollumNames.Strings = (
      'C'#243'digo'
      'Descri'#231#227'o')
    TypeFind = fFindNormal
    Left = 184
    Top = 218
  end
  object FindPlanoContaCre: TCFind
    SelectClause.Strings = (
      
        'SELECT DISTINCT cdPlanoConta, classificacao, descPlanoConta FROM' +
        ' vPlanoConta')
    JoinClause.Strings = (
      
        'WHERE stPlanoConta = '#39'ATIVO'#39' AND tpPlanoConta = [EDtpPlanoConta]' +
        ' AND idEmpresa = [DBEidEmpresa] AND naturezaPlanoConta = '#39'C'#39)
    OrderByClause.Strings = (
      'ORDER BY descPlanoConta')
    FindField = 'descPlanoConta'
    SQLConnection = dmPadrao.dbConexao
    ValueField = 'cdPlanoConta'
    CarregaDados = True
    CollumNames.Strings = (
      'C'#243'digo'
      'Classifica'#231#227'o'
      'Descri'#231#227'o')
    TypeFind = fFindNormal
    Left = 152
    Top = 218
  end
  object FindPlanoContaDeb: TCFind
    SelectClause.Strings = (
      
        'SELECT DISTINCT cdPlanoConta, classificacao, descPlanoConta FROM' +
        ' vPlanoConta')
    JoinClause.Strings = (
      
        'WHERE stPlanoConta = '#39'ATIVO'#39' AND tpPlanoConta = [EDtpPlanoConta]' +
        ' AND idEmpresa = [DBEidEmpresa] AND naturezaPlanoConta = '#39'D'#39)
    OrderByClause.Strings = (
      'ORDER BY descPlanoConta')
    FindField = 'descPlanoConta'
    SQLConnection = dmPadrao.dbConexao
    ValueField = 'cdPlanoConta'
    CarregaDados = True
    CollumNames.Strings = (
      'C'#243'digo'
      'Classifica'#231#227'o'
      'Descri'#231#227'o')
    TypeFind = fFindNormal
    Left = 120
    Top = 218
  end
end
