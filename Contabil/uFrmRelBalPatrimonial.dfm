inherited FrmRelBalPatrimonial: TFrmRelBalPatrimonial
  Caption = 'Balan'#231'o Patrimonial'
  ClientHeight = 355
  ClientWidth = 806
  ExplicitWidth = 822
  ExplicitHeight = 394
  PixelsPerInch = 96
  TextHeight = 13
  inherited Bevel3: TBevel
    Width = 806
    ExplicitWidth = 805
  end
  inherited Bevel1: TBevel
    Top = 317
    Width = 806
    ExplicitTop = 412
    ExplicitWidth = 805
  end
  inherited Bevel5: TBevel
    Width = 806
    ExplicitWidth = 805
  end
  inherited PanBotoes: TCPanelGradient
    Top = 321
    Width = 806
    TabOrder = 7
    ExplicitTop = 321
    ExplicitWidth = 806
    inherited BtnFechar: TCBitBtn
      Left = 398
      Top = 6
      ExplicitLeft = 398
      ExplicitTop = 6
    end
  end
  inherited DBEidEmpresa: TCDBEdit
    TabOrder = 0
  end
  object CGroupBox1: TCGroupBox [8]
    Left = 8
    Top = 37
    Width = 465
    Height = 180
    TabOrder = 4
    object DBRGtpPlanoConta: TCDBRadioGroup
      Left = 9
      Top = 10
      Width = 446
      Height = 44
      Caption = 'Origem'
      Columns = 2
      DataField = 'tpPlanoConta'
      DataSource = dsPadrao
      Items.Strings = (
        'Fiscal'
        'Gerencial')
      TabOrder = 0
      Values.Strings = (
        'F'
        'G')
      OnExit = DBRGtpPlanoContaExit
    end
    object GPPeriodo: TCGroupBox
      Left = 9
      Top = 60
      Width = 446
      Height = 109
      TabOrder = 1
      object CLabel1: TCLabel
        Left = 158
        Top = 14
        Width = 23
        Height = 13
        Alignment = taRightJustify
        Caption = 'Grau'
      end
      object CLabel5: TCLabel
        Left = 8
        Top = 39
        Width = 79
        Height = 13
        Alignment = taRightJustify
        Caption = 'Data de Emiss'#227'o'
      end
      object CLabel6: TCLabel
        Left = 45
        Top = 14
        Width = 42
        Height = 13
        Alignment = taRightJustify
        Caption = 'Mes/Ano'
      end
      object CLabel3: TCLabel
        Left = 173
        Top = 39
        Width = 67
        Height = 13
        Alignment = taRightJustify
        Caption = 'Encerramento'
      end
      object DBEnrGrau: TCDBEdit
        Left = 187
        Top = 11
        Width = 46
        Height = 19
        BevelKind = bkTile
        BorderStyle = bsNone
        DataField = 'nrGrau'
        DataSource = dsPadrao
        TabOrder = 1
        DecimalControl = True
        KeyMode = kmNormal
      end
      object DBEmesAnoIni: TCDBEdit
        Left = 93
        Top = 11
        Width = 59
        Height = 19
        BevelKind = bkTile
        BorderStyle = bsNone
        DataField = 'mesAnoIni'
        DataSource = dsPadrao
        TabOrder = 0
        DecimalControl = True
        KeyMode = kmNormal
      end
      object CHKOficial: TCCheckBox
        Left = 21
        Top = 61
        Width = 66
        Height = 17
        Caption = '&Oficial'
        TabOrder = 4
        OnExit = CHKOficialExit
      end
      object CHKTermoResp: TCCheckBox
        Left = 93
        Top = 61
        Width = 180
        Height = 17
        Caption = '&Listar Termo de Responsabilidade'
        TabOrder = 5
      end
      object CBListarSaldoZero: TCCheckBox
        Left = 21
        Top = 84
        Width = 218
        Height = 17
        Caption = '&Listar contas com Saldo Zero'
        TabOrder = 6
      end
      object DBEdtEmissao: TCDBEdit
        Left = 93
        Top = 36
        Width = 74
        Height = 19
        BevelKind = bkTile
        BorderStyle = bsNone
        DataField = 'dtEmissao'
        DataSource = dsPadrao
        MaxLength = 10
        TabOrder = 2
        DecimalControl = True
        KeyMode = kmDate
      end
      object DBEdtEncerramento: TCDBEdit
        Left = 246
        Top = 36
        Width = 74
        Height = 19
        BevelKind = bkTile
        BorderStyle = bsNone
        DataField = 'dtEncerramento'
        DataSource = dsPadrao
        MaxLength = 10
        TabOrder = 3
        DecimalControl = True
        KeyMode = kmDate
      end
    end
  end
  object CRadioGroup1: TCRadioGroup [9]
    Left = 705
    Top = 47
    Width = 96
    Height = 74
    Caption = 'Ordena'#231#227'o'
    ItemIndex = 0
    Items.Strings = (
      '&N'#250'merica'
      '&Alfab'#233'tica')
    TabOrder = 5
    Visible = False
  end
  object DBEListarSaldoZero: TCDBEdit [10]
    Left = 491
    Top = 238
    Width = 54
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'ListarSaldoZero'
    DataSource = dsPadrao
    TabOrder = 6
    Visible = False
    DecimalControl = True
    KeyMode = kmNormal
  end
  inherited DBEidCadEmpresa: TCDBEdit
    TabOrder = 1
  end
  object DBEDtIni: TCDBEdit [12]
    Left = 299
    Top = 259
    Width = 62
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'dtIni'
    DataSource = dsPadrao
    TabOrder = 8
    Visible = False
    DecimalControl = True
    KeyMode = kmNormal
  end
  object DBEdtFim: TCDBEdit [13]
    Left = 299
    Top = 284
    Width = 62
    Height = 19
    BevelKind = bkTile
    BorderStyle = bsNone
    DataField = 'dtFim'
    DataSource = dsPadrao
    TabOrder = 9
    Visible = False
    DecimalControl = True
    KeyMode = kmNormal
  end
  inherited imgIcones: TImageList
    Left = 552
    Top = 48
    Bitmap = {
      
      000000000000}
  end
  inherited Acao: TActionList
    Left = 520
    Top = 48
  end
  inherited dspRelatorio: TDataSetProvider
    Left = 584
    Top = 48
  end
  inherited cdsRelatorio: TClientDataSet
    CommandText = 'EXEC spContRelBalPatrimonial 1,'#39'F'#39','#39'12/2011'#39','#39'31/12/2011'#39',0,'#39'S'#39
    Left = 616
    Top = 48
    object cdsRelatorioclassificacao: TStringField
      FieldName = 'classificacao'
      Size = 30
    end
    object cdsRelatoriodescCadEmpresa: TStringField
      FieldName = 'descCadEmpresa'
      Size = 50
    end
    object cdsRelatoriocdPlanoConta: TStringField
      FieldName = 'cdPlanoConta'
      Required = True
      Size = 23
    end
    object cdsRelatoriodescPlanoConta: TStringField
      FieldName = 'descPlanoConta'
      Size = 60
    end
    object cdsRelatoriovlSaldoAnt: TFMTBCDField
      FieldName = 'vlSaldoAnt'
      Precision = 19
      Size = 4
    end
    object cdsRelatoriovlDebito: TFMTBCDField
      FieldName = 'vlDebito'
      Precision = 19
      Size = 4
    end
    object cdsRelatoriovlCredito: TFMTBCDField
      FieldName = 'vlCredito'
      Precision = 19
      Size = 4
    end
    object cdsRelatoriovlSaldoAtual: TFMTBCDField
      FieldName = 'vlSaldoAtual'
      currency = True
      Precision = 19
      Size = 4
    end
    object cdsRelatorionrGrau: TSmallintField
      FieldName = 'nrGrau'
    end
    object cdsRelatorionaturezaPlanoConta: TStringField
      FieldName = 'naturezaPlanoConta'
      FixedChar = True
      Size = 1
    end
    object cdsRelatorioidAgrupa: TStringField
      FieldName = 'idAgrupa'
      FixedChar = True
      Size = 1
    end
  end
  inherited frxDataset: TfrxDBDataset
    FieldAliases.Strings = (
      'classificacao=classificacao'
      'descCadEmpresa=descCadEmpresa'
      'cdPlanoConta=cdPlanoConta'
      'descPlanoConta=descPlanoConta'
      'vlSaldoAnt=vlSaldoAnt'
      'vlDebito=vlDebito'
      'vlCredito=vlCredito'
      'vlSaldoAtual=vlSaldoAtual'
      'nrGrau=nrGrau'
      'naturezaPlanoConta=naturezaPlanoConta'
      'idAgrupa=idAgrupa')
    OpenDataSource = True
    Left = 584
    Top = 80
  end
  inherited dsPadrao: TDataSource
    Left = 624
    Top = 208
  end
  inherited cdsPadrao: TClientDataSet
    Left = 584
    Top = 208
    object cdsPadraoidEmpresa: TIntegerField
      FieldName = 'idEmpresa'
    end
    object cdsPadraotpPlanoConta: TStringField
      FieldName = 'tpPlanoConta'
    end
    object cdsPadraonrGrau: TSmallintField
      FieldName = 'nrGrau'
    end
    object cdsPadraoListarSaldoZero: TStringField
      FieldName = 'ListarSaldoZero'
    end
    object cdsPadraomesAnoIni: TStringField
      FieldName = 'mesAnoIni'
      EditMask = '!99/9999;1;_'
    end
    object cdsPadraodtEmissao: TDateField
      FieldName = 'dtEmissao'
    end
    object cdsPadraodtEncerramento: TDateField
      FieldName = 'dtEncerramento'
    end
    object cdsPadraodtIni: TStringField
      FieldName = 'dtIni'
    end
    object cdsPadraodtFim: TStringField
      FieldName = 'dtFim'
    end
  end
  inherited frxHTMLExport1: TfrxHTMLExport
    Left = 520
    Top = 144
  end
  inherited frxTIFFExport1: TfrxTIFFExport
    Left = 552
    Top = 112
  end
  inherited dspLookUp: TDataSetProvider
    Left = 520
    Top = 208
  end
  inherited cdsLookUp: TClientDataSet
    Left = 552
    Top = 208
  end
  inherited frxRTFExport1: TfrxRTFExport
    Left = 584
    Top = 112
  end
  inherited frxJPEGExport1: TfrxJPEGExport
    Left = 520
    Top = 112
  end
  inherited cdsEmpresa: TClientDataSet
    Left = 584
  end
  inherited dsEmpresa: TDataSource
    Left = 616
  end
  object frxRel: TfrxReport
    Version = '5.1.5'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.MDIChild = True
    PreviewOptions.Modal = False
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.Author = 'Gesys Sistemas de Informa'#231#227'o'
    ReportOptions.CreateDate = 39263.419322395800000000
    ReportOptions.LastChange = 41578.713411284720000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'Var eSldAnt, eDeb, eCred, eSldAtual, vlTotal: Extended;'
      ''
      'procedure MasterData1OnAfterPrint(Sender: TfrxComponent);'
      'begin'
      '  If (<frxDataset."nrGrau"> = 1) Then'
      '     Begin'
      '       if (<frxDataset."naturezaPlanoConta"> = '#39'D'#39') then'
      '         eSldAnt   := eSldAnt   - <frxDataset."vlSaldoAnt">  '
      '       else'
      
        '         eSldAnt   := eSldAnt   + <frxDataset."vlSaldoAnt">;    ' +
        '         '
      '       eDeb      := eDeb      + <frxDataset."vlDebito">;'
      '       eCred     := eCred     + <frxDataset."vlCredito">;'
      '       eSldAtual := eSldAtual + <frxDataset."vlSaldoAtual">;'
      
        '       vlTotal := <frxDataset."vlSaldoAtual">;                  ' +
        '                          '
      '     End;'
      'end;'
      ''
      ''
      ''
      'procedure Memo12OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      
        ' if (<frxDataset."nrGrau"> = '#39'1'#39') OR (<frxDataset."nrGrau"> = '#39'2' +
        #39') then                      '
      '    Memo12.Font.Style := fsBold  '
      '  else'
      '    Memo12.Font.Style := 0;'
      '  if (<frxDataset."nrGrau"> = '#39'3'#39') then'
      '  begin'
      
        '    MasterData1.Height := 25;                                   ' +
        '                                    '
      '  end;  '
      'end;'
      ''
      ''
      'procedure Memo15OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      '  if (<frxDataset."idAgrupa"> = '#39'1'#39') then'
      '    Memo15.Lines.Text := '#39'TOTAL DO ATIVO'#39
      '  else'
      
        '    Memo15.Lines.Text := '#39'TOTAL DO PASSIVO'#39';                    ' +
        '                            '
      'end;'
      ''
      'begin'
      '  eSldAnt   := 0;'
      '  eDeb      := 0;'
      '  eCred     := 0;'
      '  eSldAtual := 0;                                        '
      'end.')
    Left = 552
    Top = 80
    Datasets = <
      item
        DataSet = frxDataset
        DataSetName = 'frxDataset'
      end
      item
        DataSet = FrxDatasetCont
        DataSetName = 'frxDatasetCont'
      end
      item
        DataSet = frxDBPagina
        DataSetName = 'frxPagina'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 15.118110240000000000
        Top = 245.669450000000000000
        Width = 793.701300000000000000
        OnAfterPrint = 'MasterData1OnAfterPrint'
        OnBeforePrint = 'MasterData1OnBeforePrint'
        DataSet = frxDataset
        DataSetName = 'frxDataset'
        RowCount = 0
        object Memo12: TfrxMemoView
          Left = 39.181090159999990000
          Top = 0.007729999999980919
          Width = 234.661375830000000000
          Height = 13.228346460000000000
          OnBeforePrint = 'Memo12OnBeforePrint'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descPlanoConta">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo22: TfrxMemoView
          Left = 642.874477090000000000
          Width = 75.590600000000000000
          Height = 13.228346460000000000
          DataField = 'vlSaldoAtual'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDataset."vlSaldoAtual"]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
      end
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        Height = 121.874072500000000000
        Top = 18.897650000000000000
        Width = 793.701300000000000000
        object Line2: TfrxLineView
          Left = 18.519685039370100000
          Top = 119.102350000000000000
          Width = 706.771636460000000000
          Color = clBlack
          Frame.Typ = [ftTop]
          Frame.Width = 0.100000000000000000
        end
        object MemdescEmpresa: TfrxMemoView
          Left = 29.118120000000000000
          Top = 56.015770000000010000
          Width = 495.118430000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Empresa')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo1: TfrxMemoView
          Left = 18.897650000000000000
          Top = 41.614236690000000000
          Width = 702.992580000000000000
          Height = 18.897637800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'Balan'#231'o Patrimonial')
          ParentFont = False
          WordWrap = False
        end
        object Memo23: TfrxMemoView
          Left = 291.803340000000000000
          Top = 101.330860000000000000
          Width = 68.031540000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Encerrado em:')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object MemPeriodo: TfrxMemoView
          Left = 382.598640000000000000
          Top = 101.330860000000000000
          Width = 34.015770000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'Periodo')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Line3: TfrxLineView
          Left = 18.653540870000000000
          Top = 98.315090000000000000
          Width = 706.771631570000000000
          Color = clBlack
          Diagonal = True
        end
        object MemdtEmissao: TfrxMemoView
          Left = 645.094930000000000000
          Top = 76.094620000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'Emissao')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo14: TfrxMemoView
          Left = 565.724799999999900000
          Top = 76.094620000000000000
          Width = 83.149660000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Data de Emissao:')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo17: TfrxMemoView
          Left = 605.181510000000000000
          Top = 62.519790000000000000
          Width = 41.574830000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'P'#225'gina:')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo13: TfrxMemoView
          Left = 645.094930000000000000
          Top = 62.519790000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[((<frxPagina."pagFinal">+<Page>)-1)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo10: TfrxMemoView
          Left = 28.677180000000000000
          Top = 75.417440000000000000
          Width = 321.260050000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Nilus Gest'#227'o do Agronegocio')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Line6: TfrxLineView
          Left = 18.677162910000000000
          Top = 31.440940000000000000
          Width = 706.771636460000000000
          Color = clBlack
          Frame.Typ = [ftTop]
          Frame.Width = 0.100000000000000000
        end
        object Line7: TfrxLineView
          Left = 18.897650000000000000
          Top = 32.385900000000000000
          Height = 64.251961180000000000
          Color = clBlack
          Diagonal = True
        end
        object Line8: TfrxLineView
          Left = 723.685530000000000000
          Top = 30.944959999999990000
          Height = 68.031491180000000000
          Color = clBlack
          Diagonal = True
        end
        object Memo11: TfrxMemoView
          Left = 648.945270000000000000
          Top = 100.149660000000000000
          Width = 30.236240000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Grau:')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object MemnrGrau: TfrxMemoView
          Left = 683.315400000000000000
          Top = 100.149660000000000000
          Width = 37.795300000000000000
          Height = 15.118120000000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            'Periodo')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        Height = 200.315090000000000000
        Top = 359.055350000000000000
        Width = 793.701300000000000000
        object MDescObs: TfrxMemoView
          Left = 19.653543310000000000
          Top = 2.677180000000021000
          Width = 706.772110000000000000
          Height = 90.708720000000000000
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haBlock
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo2: TfrxMemoView
          Left = 37.795300000000000000
          Top = 141.732283460000000000
          Width = 208.204665830000000000
          Height = 13.228346460000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDatasetCont."descRepresentante">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo3: TfrxMemoView
          Left = 37.795300000000000000
          Top = 156.850393700000000000
          Width = 208.204665830000000000
          Height = 13.228346460000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDatasetCont."cargoRepresentante">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo4: TfrxMemoView
          Left = 37.795300000000000000
          Top = 171.968503937007900000
          Width = 208.204665830000000000
          Height = 13.228346460000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDatasetCont."idCnpjCpfRepresentante">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo5: TfrxMemoView
          Left = 476.220780000000000000
          Top = 141.732283460000000000
          Width = 208.204665830000000000
          Height = 13.228346460000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDatasetCont."descContador">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo6: TfrxMemoView
          Left = 476.220780000000000000
          Top = 156.850393700000000000
          Width = 208.204665830000000000
          Height = 13.228346460000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'CONTADOR')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo9: TfrxMemoView
          Left = 476.220780000000000000
          Top = 171.968503937007900000
          Width = 208.204665830000000000
          Height = 13.228346460000000000
          AutoWidth = True
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDatasetCont."crcContador">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Line1: TfrxLineView
          Left = 22.677180000000000000
          Top = 137.724490000000100000
          Width = 245.669450000000000000
          Color = clBlack
          Diagonal = True
        end
        object Line4: TfrxLineView
          Left = 457.323130000000000000
          Top = 137.724490000000100000
          Width = 245.669450000000000000
          Color = clBlack
          Diagonal = True
        end
      end
      object GroupHeader1: TfrxGroupHeader
        FillType = ftBrush
        Height = 22.677180000000000000
        Top = 200.315090000000000000
        Width = 793.701300000000000000
        Condition = 'frxDataset."idAgrupa"'
        StartNewPage = True
        object Shape1: TfrxShapeView
          Left = 26.677162910000000000
          Top = 0.629870000000011000
          Width = 699.212576460000000000
          Height = 18.897650000000000000
          Fill.BackColor = 15790320
          Frame.Color = 15790320
        end
        object Memo8: TfrxMemoView
          Left = 30.181077950000000000
          Top = 0.629869999999982600
          Width = 232.677180000000000000
          Height = 18.897637800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Descri'#231#227'o')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo21: TfrxMemoView
          Left = 664.213050000000000000
          Top = 0.677179999999992800
          Width = 57.606370000000000000
          Height = 18.897637800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Saldo Atual')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
      end
      object GroupFooter1: TfrxGroupFooter
        FillType = ftBrush
        Height = 15.118120000000000000
        Top = 283.464750000000000000
        Width = 793.701300000000000000
        object Memo15: TfrxMemoView
          Left = 374.189240000000000000
          Top = 0.007729999999980919
          Width = 234.661375830000000000
          Height = 13.228346460000000000
          OnBeforePrint = 'Memo15OnBeforePrint'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo16: TfrxMemoView
          Left = 642.488686930000000000
          Width = 75.590600000000000000
          Height = 13.228346460000000000
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[vlTotal]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
      end
      object PageFooter1: TfrxPageFooter
        FillType = ftBrush
        Height = 44.133890000000000000
        Top = 582.047620000000000000
        Visible = False
        Width = 793.701300000000000000
        object MRodape: TfrxMemoView
          Left = 26.456692910000000000
          Top = 7.181102359999954000
          Width = 275.905690000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Nilus Solu'#231#245'es para o Agroneg'#243'cio Ltda.')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Line5: TfrxLineView
          Left = 26.456692910000000000
          Top = 3.401359999999954000
          Width = 706.771653543307000000
          Color = clBlack
          Frame.Typ = [ftTop]
          Frame.Width = 0.100000000000000000
        end
      end
    end
  end
  object ACBRExtenso: TACBrExtenso
    StrMoeda = 'Real'
    StrMoedas = 'Reais'
    StrCentavo = 'Centavo'
    StrCentavos = 'Centavos'
    Left = 416
    Top = 272
  end
  object cdsContador: TClientDataSet
    Aggregates = <>
    CommandText = 
      'SELECT descContador, crcContador, descRepresentante, idCnpjCpfCo' +
      'ntador, cargoRepresentante, idCnpjCpfRepresentante FROM EmpresaC' +
      'ontabil'
    Params = <>
    ProviderName = 'dspRelatorio'
    Left = 616
    Top = 256
  end
  object dspContador: TDataSetProvider
    DataSet = dmPadrao.sdsRelatorio
    Options = [poAllowCommandText, poUseQuoteChar]
    Left = 584
    Top = 256
  end
  object FrxDatasetCont: TfrxDBDataset
    UserName = 'frxDatasetCont'
    CloseDataSource = False
    FieldAliases.Strings = (
      'descContador=descContador'
      'crcContador=crcContador'
      'descRepresentante=descRepresentante'
      'idCnpjCpfContador=idCnpjCpfContador'
      'cargoRepresentante=cargoRepresentante'
      'idCnpjCpfRepresentante=idCnpjCpfRepresentante')
    DataSet = cdsContador
    BCDToCurrency = False
    Left = 536
    Top = 256
  end
  object cdsPagina: TClientDataSet
    Aggregates = <>
    CommandText = 'SELECT ISNULL(pagFinal, 0) pagFinal FROM ContPagina WHERE 1=2'
    Params = <>
    ProviderName = 'dspPagina'
    Left = 752
    Top = 176
  end
  object dspPagina: TDataSetProvider
    DataSet = dmPadrao.sdsRelatorio
    Options = [poAllowCommandText, poUseQuoteChar]
    Left = 720
    Top = 176
  end
  object frxDBPagina: TfrxDBDataset
    UserName = 'frxPagina'
    CloseDataSource = False
    FieldAliases.Strings = (
      'pagFinal=pagFinal')
    OpenDataSource = False
    DataSet = cdsPagina
    BCDToCurrency = False
    Left = 688
    Top = 176
  end
end
