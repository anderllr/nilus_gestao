inherited FrmRelTermoAbertura: TFrmRelTermoAbertura
  Caption = 'Relat'#243'rio de Termo de Abertura/Encerramento'
  ClientHeight = 342
  ClientWidth = 660
  Color = clWindow
  ExplicitWidth = 676
  ExplicitHeight = 380
  PixelsPerInch = 96
  TextHeight = 13
  inherited Bevel3: TBevel
    Width = 660
    ExplicitWidth = 874
  end
  inherited Bevel1: TBevel
    Top = 304
    Width = 660
    ExplicitTop = 337
    ExplicitWidth = 874
  end
  inherited Bevel5: TBevel
    Width = 660
    ExplicitWidth = 874
  end
  inherited PanBotoes: TCPanelGradient
    Top = 308
    Width = 660
    TabOrder = 5
    ExplicitTop = 308
    ExplicitWidth = 660
    inherited BtnGerar: TCBitBtn
      Top = 6
      ExplicitTop = 6
    end
    inherited BtnFechar: TCBitBtn
      Left = 387
      Top = 5
      ExplicitLeft = 387
      ExplicitTop = 5
    end
  end
  inherited LookdescCadEmpresa: TCLookUp
    TabOrder = 2
  end
  inherited LookdescAbreviada: TCLookUp
    TabOrder = 3
  end
  object GPPeriodo: TCGroupBox [8]
    Left = 8
    Top = 99
    Width = 446
    Height = 63
    TabOrder = 4
    object CLabel3: TCLabel
      Left = 11
      Top = 37
      Width = 63
      Height = 13
      Alignment = taRightJustify
      Caption = 'Numero Livro'
    end
    object CLabel6: TCLabel
      Left = 38
      Top = 12
      Width = 36
      Height = 13
      Alignment = taRightJustify
      Caption = 'Per'#237'odo'
    end
    object CLabel7: TCLabel
      Left = 166
      Top = 12
      Width = 16
      Height = 13
      Alignment = taRightJustify
      Caption = 'at'#233
    end
    object CLabel1: TCLabel
      Left = 166
      Top = 37
      Width = 64
      Height = 13
      Alignment = taRightJustify
      Caption = 'Data Emiss'#227'o'
    end
    object DBEidNrLivro: TCDBEdit
      Left = 80
      Top = 35
      Width = 80
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'idNrLivro'
      DataSource = dsPadrao
      TabOrder = 2
      DecimalControl = True
      KeyMode = kmNormal
    end
    object DBEdtInicial: TCDBEdit
      Left = 80
      Top = 10
      Width = 80
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'dtInicial'
      DataSource = dsPadrao
      MaxLength = 10
      TabOrder = 0
      DecimalControl = True
      KeyMode = kmDate
    end
    object DBEdtFinal: TCDBEdit
      Left = 188
      Top = 10
      Width = 80
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'dtFinal'
      DataSource = dsPadrao
      MaxLength = 10
      TabOrder = 1
      DecimalControl = True
      KeyMode = kmDate
    end
    object CHKOficial: TCCheckBox
      Left = 322
      Top = 36
      Width = 57
      Height = 19
      Caption = 'Oficial'
      TabOrder = 4
    end
    object DBEdtEmissao: TCDBEdit
      Left = 236
      Top = 35
      Width = 80
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'dtEmissao'
      DataSource = dsPadrao
      MaxLength = 10
      TabOrder = 3
      DecimalControl = True
      KeyMode = kmDate
    end
  end
  object DBRGOrigem: TCDBRadioGroup [9]
    Left = 8
    Top = 35
    Width = 446
    Height = 58
    Caption = 'Origem'
    Columns = 2
    DataField = 'origem'
    DataSource = dsPadrao
    Items.Strings = (
      'Abertura D'#237'ario'
      'Encerramento D'#237'ario'
      'Abertura Raz'#227'o'
      'Encerramento Raz'#227'o')
    ParentBackground = True
    TabOrder = 1
    Values.Strings = (
      'Abertura D'#237'ario'
      'Encerramento D'#237'ario'
      'Abertura Raz'#227'o'
      'Encerramento Raz'#227'o')
  end
  inherited imgIcones: TImageList
    Bitmap = {
      
      000000000000}
  end
  inherited cdsRelatorio: TClientDataSet
    CommandText = 
      'SELECT A.idCnpjCpf, A.descCadAdicional, A.bairro, A.endereco, A.' +
      'idInscEstadual, cid.descCidade, cid.idEstado, Cont.idNire, Cont.' +
      'dtRegistro, C.cargoRepresentante, C.descRepresentante, C.idCnpjC' +
      'pfRepresentante, C.descContador, C.crcContador, C.rg, C.OrgaoExp' +
      ', C.rgRep, C.OrgaoExpRep FROM CadAdicional A INNER JOIN EmpresaC' +
      'ontabil C ON C.idCadGeral = A.idCadGeral INNER JOIN Cidade cid O' +
      'N A.idCidade = cid.idCidade INNER JOIN ConfigContabil Cont ON Co' +
      'nt.idCadGeral = A.idCadGeral'
  end
  inherited frxDataset: TfrxDBDataset
    FieldAliases.Strings = (
      'idCnpjCpf=idCnpjCpf'
      'descCadAdicional=descCadAdicional'
      'bairro=bairro'
      'endereco=endereco'
      'idInscEstadual=idInscEstadual'
      'descCidade=descCidade'
      'idEstado=idEstado'
      'idNire=idNire'
      'dtRegistro=dtRegistro'
      'cargoRepresentante=cargoRepresentante'
      'descRepresentante=descRepresentante'
      'idCnpjCpfRepresentante=idCnpjCpfRepresentante'
      'descContador=descContador'
      'crcContador=crcContador'
      'rg=rg'
      'OrgaoExp=OrgaoExp'
      'rgRep=rgRep'
      'OrgaoExpRep=OrgaoExpRep')
  end
  inherited cdsPadrao: TClientDataSet
    AfterInsert = cdsPadraoAfterInsert
    object cdsPadraoidEmpresa: TIntegerField
      FieldName = 'idEmpresa'
    end
    object cdsPadraodtInicial: TDateField
      FieldName = 'dtInicial'
    end
    object cdsPadraodtFinal: TDateField
      FieldName = 'dtFinal'
    end
    object cdsPadraoidNrLivro: TIntegerField
      FieldName = 'idNrLivro'
    end
    object cdsPadraodtEmissao: TDateField
      FieldName = 'dtEmissao'
    end
    object cdsPadraoorigem: TStringField
      FieldName = 'origem'
    end
    object cdsPadraocdPlanoConta: TIntegerField
      FieldName = 'cdPlanoConta'
    end
  end
  object frxRel: TfrxReport
    Version = '4.10.5'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.MDIChild = True
    PreviewOptions.Modal = False
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.Author = 'Gesys Sistemas de Informa'#231#227'o'
    ReportOptions.CreateDate = 39263.419322395800000000
    ReportOptions.LastChange = 41212.597972719900000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 488
    Top = 56
    Datasets = <
      item
        DataSet = frxDataset
        DataSetName = 'frxDataset'
      end
      item
        DataSet = frxDBPagina
        DataSetName = 'frxPagina'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      object PageHeader1: TfrxPageHeader
        Height = 152.110312500000000000
        Top = 18.897650000000000000
        Width = 793.701300000000000000
        object Memo1: TfrxMemoView
          Left = 0.239406400000000000
          Top = 107.716586690000000000
          Width = 793.701300000000000000
          Height = 34.015770000000000000
          ShowHint = False
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -24
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'Termo de Abertura')
          ParentFont = False
          WordWrap = False
        end
        object Memo11: TfrxMemoView
          Left = 604.724800000000000000
          Top = 26.456710000000000000
          Width = 56.692950000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'P'#225'gina:')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MPage: TfrxMemoView
          Left = 668.976810000000000000
          Top = 26.456710000000000000
          Width = 56.692950000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            '[((<frxPagina."pagFinal">+<Page>)-1)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
      end
      object MasterData1: TfrxMasterData
        Height = 767.244590000000000000
        Top = 230.551330000000000000
        Width = 793.701300000000000000
        DataSet = frxDataset
        DataSetName = 'frxDataset'
        RowCount = 0
        Stretched = True
        object Memo2: TfrxMemoView
          Left = 53.370130000000000000
          Top = 94.007959449999990000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Raz'#227'o Social: ')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo3: TfrxMemoView
          Left = 53.370130000000000000
          Top = 124.244199450000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Endere'#231'o:')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo4: TfrxMemoView
          Left = 53.370130000000000000
          Top = 151.700909450000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Bairro: ')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo5: TfrxMemoView
          Left = 53.370130000000000000
          Top = 179.378089450000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Cidade:')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo6: TfrxMemoView
          Left = 53.370130000000000000
          Top = 205.055269450000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Estado:')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo7: TfrxMemoView
          Left = 53.118120000000000000
          Top = 230.732449450000000000
          Width = 158.740260000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Registro na Junta (NIRE): ')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo8: TfrxMemoView
          Left = 53.118120000000000000
          Top = 256.630099450000000000
          Width = 158.740260000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Data do Registro: ')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo9: TfrxMemoView
          Left = 53.118120000000000000
          Top = 283.307279450000000000
          Width = 158.740260000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Inscri'#231#227'o Estadual:')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo10: TfrxMemoView
          Left = 53.118120000000000000
          Top = 312.984459450000000000
          Width = 158.740260000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'C.N.P.J./C.P.F.:')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MRazao: TfrxMemoView
          Left = 253.433210000000000000
          Top = 93.984230000000420000
          Width = 487.559028270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descCadAdicional">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MEndereco: TfrxMemoView
          Left = 253.433210000000000000
          Top = 124.220450470000000000
          Width = 487.559028270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."endereco">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MBairro: TfrxMemoView
          Left = 253.433210000000000000
          Top = 151.677143390000000000
          Width = 487.559028270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."bairro">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MCidade: TfrxMemoView
          Left = 253.433210000000000000
          Top = 179.354308740000000000
          Width = 487.559028270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descCidade">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MEstado: TfrxMemoView
          Left = 253.433210000000000000
          Top = 205.031474100000000000
          Width = 487.559028270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."idEstado">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MRegistro: TfrxMemoView
          Left = 253.433210000000000000
          Top = 230.708639450000000000
          Width = 487.559028270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."idNire">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MDtRegistro: TfrxMemoView
          Left = 253.433210000000000000
          Top = 256.629899290000000000
          Width = 487.559028270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."dtRegistro">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MInsc: TfrxMemoView
          Left = 253.433210000000000000
          Top = 283.307064650000000000
          Width = 487.559028270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."idInscEstadual">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MCnpj: TfrxMemoView
          Left = 253.433210000000000000
          Top = 312.984230000000000000
          Width = 487.559028270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."idCnpjCpf">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MLocal: TfrxMemoView
          Left = 52.913420000000000000
          Top = 387.575059450000000000
          Width = 207.873808270000000000
          Height = 18.897650000000000000
          ShowHint = False
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = 'dd mmm yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MEmp: TfrxMemoView
          Left = 57.897650000000000000
          Top = 529.535789450000100000
          Width = 234.330518270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descRepresentante">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MOcupacaoEmp: TfrxMemoView
          Left = 57.897650000000000000
          Top = 549.551559450000000000
          Width = 234.330518270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."cargoRepresentante">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MRGEmp: TfrxMemoView
          Left = 57.897650000000000000
          Top = 570.008269449999900000
          Width = 234.330518270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'RG: [(<frxDataset."rgRep">)] - [(<frxDataset."OrgaoExpRep">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MCPFEmp: TfrxMemoView
          Left = 57.897650000000000000
          Top = 588.464979450000000000
          Width = 234.330518270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'CPF: [(<frxDataset."idCnpjCpfRepresentante">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Line2: TfrxLineView
          Left = 51.897650000000000000
          Top = 523.638139450000100000
          Width = 291.023810000000000000
          ShowHint = False
          Diagonal = True
        end
        object MdescContador: TfrxMemoView
          Left = 458.527830000000000000
          Top = 529.535789450000100000
          Width = 234.330518270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descContador">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MOcupacaoCont: TfrxMemoView
          Left = 458.527830000000000000
          Top = 546.551559450000000000
          Width = 234.330518270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'CONTADOR')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MrgCont: TfrxMemoView
          Left = 458.527830000000000000
          Top = 566.008269449999900000
          Width = 234.330518270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'RG: [(<frxDataset."rg">)] - [(<frxDataset."OrgaoExp">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object MCRCCont: TfrxMemoView
          Left = 458.527830000000000000
          Top = 585.464979450000000000
          Width = 234.330518270000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'CRC: [(<frxDataset."crcContador">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Line3: TfrxLineView
          Left = 452.527830000000000000
          Top = 523.638139450000100000
          Width = 291.023810000000000000
          ShowHint = False
          Diagonal = True
        end
        object MDesc: TfrxMemoView
          Left = 52.913420000000000000
          Top = 7.559059999999988000
          Width = 687.874118270000000000
          Height = 71.811070000000000000
          ShowHint = False
          StretchMode = smMaxHeight
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Courier'
          Font.Style = []
          HAlign = haBlock
          ParentFont = False
        end
      end
      object PageFooter1: TfrxPageFooter
        Height = 25.236240000000000000
        Top = 1058.268400000000000000
        Visible = False
        Width = 793.701300000000000000
        object MRodape: TfrxMemoView
          Left = 26.456692910000000000
          Top = 7.181102360000068000
          Width = 275.905690000000000000
          Height = 11.338590000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Nilus Solu'#231#245'es para o Agroneg'#243'cio Ltda.')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Line8: TfrxLineView
          Left = 26.456692910000000000
          Top = 3.401359999999841000
          Width = 733.228346460000000000
          ShowHint = False
          Frame.Typ = [ftTop]
          Frame.Width = 0.100000000000000000
        end
      end
    end
  end
  object ExtensoLivro: TACBrExtenso
    StrMoeda = 'Real'
    StrMoedas = 'Reais'
    StrCentavo = 'Centavo'
    StrCentavos = 'Centavos'
    ZeroAEsquerda = False
    Left = 688
    Top = 192
  end
  object ExtensoPaginas: TACBrExtenso
    StrMoeda = 'Real'
    StrMoedas = 'Reais'
    StrCentavo = 'Centavo'
    StrCentavos = 'Centavos'
    ZeroAEsquerda = False
    Left = 688
    Top = 240
  end
  object FindPlanoConta: TCFind
    SelectClause.Strings = (
      
        'SELECT DISTINCT cdPlanoConta, classificacao, descPlanoConta FROM' +
        ' vPlanoConta')
    JoinClause.Strings = (
      
        'WHERE stPlanoConta = '#39'ATIVO'#39' AND tpPlanoConta = '#39'F'#39' AND idEmpres' +
        'a = [DBEidEmpresa]')
    OrderByClause.Strings = (
      'ORDER BY descPlanoConta')
    FindField = 'descPlanoConta'
    SQLConnection = dmPadrao.dbConexao
    ValueField = 'cdPlanoConta'
    CarregaDados = True
    CollumNames.Strings = (
      'C'#243'digo'
      'Classifica'#231#227'o'
      'Descri'#231#227'o')
    TypeFind = fFindNormal
    Left = 656
    Top = 50
  end
  object frxDBPagina: TfrxDBDataset
    UserName = 'frxPagina'
    CloseDataSource = False
    FieldAliases.Strings = (
      'pagFinal=pagFinal')
    OpenDataSource = False
    DataSet = cdsPagina
    BCDToCurrency = False
    Left = 445
    Top = 232
  end
  object dspPagina: TDataSetProvider
    DataSet = dmPadrao.sdsRelatorio
    Options = [poAllowCommandText, poUseQuoteChar]
    Left = 477
    Top = 232
  end
  object cdsPagina: TClientDataSet
    Aggregates = <>
    CommandText = 'SELECT ISNULL(pagFinal, 0) pagFinal FROM ContPagina WHERE 1=2'
    Params = <>
    ProviderName = 'dspPagina'
    Left = 509
    Top = 232
  end
end
