inherited FrmPesSegMenu: TFrmPesSegMenu
  Caption = 'Pesquisa de Menus do Sistema'
  ClientHeight = 413
  ClientWidth = 558
  ExplicitWidth = 574
  ExplicitHeight = 451
  PixelsPerInch = 96
  TextHeight = 13
  inherited GrupoGrid: TCGroupBox
    Top = 138
    Width = 558
    Height = 275
    TabOrder = 2
    ExplicitTop = 138
    ExplicitWidth = 558
    ExplicitHeight = 275
    inherited DBGrid: TCDBGrid
      Width = 554
      Height = 258
      Columns = <
        item
          Expanded = False
          FieldName = 'idSegModulo'
          Title.Caption = 'M'#243'dulo'
          Width = 50
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'idSegGrupoMenu'
          Title.Caption = 'Grupo'
          Width = 50
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'idSegMenu'
          Title.Caption = 'C'#243'digo'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'descSegMenu'
          Title.Caption = 'Descri'#231#227'o'
          Width = 320
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'descSegMenuInterno'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'stSegMenu'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'excluir'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'acessar'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'incluir'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'editar'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'obsSegMenu'
          Visible = False
        end>
    end
  end
  inherited PanBotoes: TCPanelGradient
    Top = 105
    Width = 558
    Visible = False
    ExplicitTop = 99
    ExplicitWidth = 558
  end
  inherited PanTopo: TCPanelGradient
    Width = 558
    Height = 105
    TabOrder = 0
    Visible = False
    ExplicitTop = -2
    ExplicitWidth = 558
    ExplicitHeight = 105
    object CLabel58: TCLabel
      Left = 139
      Top = 68
      Width = 46
      Height = 13
      Alignment = taRightJustify
      Caption = 'Descri'#231#227'o'
    end
    object CLabel3: TCLabel
      Left = 18
      Top = 68
      Width = 33
      Height = 13
      Alignment = taRightJustify
      Caption = 'Codigo'
    end
    object CLabel1: TCLabel
      Left = 18
      Top = 43
      Width = 73
      Height = 13
      Alignment = taRightJustify
      Caption = 'Grupo de Menu'
    end
    object CLabel61: TCLabel
      Left = 18
      Top = 18
      Width = 34
      Height = 13
      Alignment = taRightJustify
      Caption = 'M'#243'dulo'
    end
    object DBEdescSegMenu: TCDBEdit
      Left = 190
      Top = 65
      Width = 348
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'descSegMenu'
      DataSource = dsPadrao
      TabOrder = 5
      DecimalControl = True
      KeyMode = kmNormal
    end
    object DBEidSegMenu: TCDBEdit
      Left = 57
      Top = 65
      Width = 72
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'idSegMenu'
      DataSource = dsPadrao
      TabOrder = 4
      Find = dmFind.FindSegMenu
      DecimalControl = True
      KeyMode = kmUSUpper
    end
    object DBEdescSegGrupoMenu: TCLookUp
      Left = 175
      Top = 40
      Width = 363
      Height = 19
      Alignment = taLeftJustify
      BevelInner = bvSpace
      BevelOuter = bvLowered
      Color = 14149350
      ParentBackground = False
      TabOrder = 3
      Key.Strings = (
        'idSegModulo'
        'idSegGrupoMenu')
      LookUpKey.Strings = (
        'idSegModulo'
        'idSegGrupoMenu')
      ClientDataSet = cdsLookUp
      DataSource = dsPadrao
      ValidaCampoObrigatorio = False
      CampoObrigatorio = True
      LookUpTable = 'SegGrupoMenu'
      ReturnField = 'descSegGrupoMenu'
    end
    object DBEidSegGrupoMenu: TCDBEdit
      Left = 97
      Top = 40
      Width = 72
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'idSegGrupoMenu'
      DataSource = dsPadrao
      TabOrder = 2
      Find = dmFind.FindSegGrupoMenu
      AcaoCad = frmPrincipal.actCadSegGrupoMenu
      DecimalControl = True
      KeyMode = kmNormal
    end
    object LookSegModulo: TCLookUp
      Left = 117
      Top = 15
      Width = 260
      Height = 19
      Alignment = taLeftJustify
      BevelInner = bvSpace
      BevelOuter = bvLowered
      Color = 14149350
      ParentBackground = False
      TabOrder = 1
      Key.Strings = (
        'idSegModulo')
      LookUpKey.Strings = (
        'idSegModulo')
      ClientDataSet = cdsLookUp
      DataSource = dsPadrao
      ValidaCampoObrigatorio = False
      CampoObrigatorio = True
      LookUpTable = 'SegModulo'
      ReturnField = 'descSegModulo'
      object CLookUp1: TCLookUp
        Left = 0
        Top = 16
        Width = 260
        Height = 19
        Alignment = taLeftJustify
        BevelInner = bvSpace
        BevelOuter = bvLowered
        Color = 14149350
        ParentBackground = False
        TabOrder = 0
        Key.Strings = (
          'idSegModulo')
        LookUpKey.Strings = (
          'idSegModulo')
        ClientDataSet = cdsLookUp
        DataSource = dsPadrao
        ValidaCampoObrigatorio = False
        CampoObrigatorio = True
        LookUpTable = 'SegModulo'
        ReturnField = 'descSegModulo'
      end
    end
    object DBEidSegModulo: TCDBEdit
      Left = 58
      Top = 15
      Width = 53
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'idSegModulo'
      DataSource = dsPadrao
      TabOrder = 0
      Find = dmFind.FindSegModulo
      AcaoCad = frmPrincipal.actCadSegModulo
      DecimalControl = True
      KeyMode = kmNormal
    end
  end
  inherited imgIcones: TImageList
    Bitmap = {
      
      000000000000}
  end
  inherited dspLookUp: TDataSetProvider
    Left = 168
    Top = 192
  end
  inherited cdsLookUp: TClientDataSet
    Left = 256
    Top = 200
  end
  inherited dspPadrao: TDataSetProvider
    Left = 192
    Top = 256
  end
  inherited cdsPadrao: TClientDataSet
    ProviderName = ''
    Left = 320
    Top = 256
    object cdsPadraoidSegModulo: TIntegerField
      FieldName = 'idSegModulo'
    end
    object cdsPadraoidSegGrupoMenu: TIntegerField
      FieldName = 'idSegGrupoMenu'
    end
    object cdsPadraoidSegMenu: TIntegerField
      FieldName = 'idSegMenu'
    end
    object cdsPadraodescSegMenu: TStringField
      FieldName = 'descSegMenu'
      Size = 100
    end
  end
  inherited dsPadrao: TDataSource
    Left = 408
    Top = 256
  end
  inherited cdsGrid: TClientDataSet
    CommandText = 'SELECT * FROM SegMenu'
    Left = 320
    Top = 312
    object cdsGrididSegModulo: TSmallintField
      FieldName = 'idSegModulo'
      Required = True
    end
    object cdsGrididSegGrupoMenu: TSmallintField
      FieldName = 'idSegGrupoMenu'
      Required = True
    end
    object cdsGrididSegMenu: TSmallintField
      FieldName = 'idSegMenu'
      Required = True
    end
    object cdsGriddescSegMenu: TStringField
      FieldName = 'descSegMenu'
      Required = True
      Size = 70
    end
    object cdsGriddescSegMenuInterno: TStringField
      FieldName = 'descSegMenuInterno'
      Required = True
      Size = 70
    end
    object cdsGridstSegMenu: TStringField
      FieldName = 'stSegMenu'
      Required = True
      Size = 15
    end
    object cdsGridexcluir: TStringField
      FieldName = 'excluir'
      Required = True
      FixedChar = True
      Size = 1
    end
    object cdsGridacessar: TStringField
      FieldName = 'acessar'
      Required = True
      FixedChar = True
      Size = 1
    end
    object cdsGridincluir: TStringField
      FieldName = 'incluir'
      Required = True
      FixedChar = True
      Size = 1
    end
    object cdsGrideditar: TStringField
      FieldName = 'editar'
      Required = True
      FixedChar = True
      Size = 1
    end
    object cdsGridobsSegMenu: TStringField
      FieldName = 'obsSegMenu'
      Required = True
      Size = 250
    end
  end
  inherited dsGrid: TDataSource
    Left = 408
    Top = 312
  end
  inherited dspGrid: TDataSetProvider
    Left = 224
    Top = 312
  end
  inherited sdsPadrao: TSQLDataSet
    Left = 120
    Top = 256
  end
  inherited sdsGrid: TSQLDataSet
    Top = 312
  end
end
