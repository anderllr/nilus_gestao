inherited FrmRelListagemCliFor: TFrmRelListagemCliFor
  Caption = 'Listagem de Clientes e Fornecedores'
  ClientHeight = 562
  ClientWidth = 1184
  ExplicitWidth = 1200
  ExplicitHeight = 601
  PixelsPerInch = 96
  TextHeight = 13
  inherited Bevel1: TBevel
    Left = 2
    Top = 101
    Width = 1172
    Align = alCustom
    ExplicitLeft = 2
    ExplicitTop = 101
    ExplicitWidth = 1172
  end
  inherited PanBotoes: TCPanelGradient
    Left = -2
    Top = 105
    Width = 1175
    Align = alCustom
    TabOrder = 2
    ExplicitLeft = -2
    ExplicitTop = 105
    ExplicitWidth = 1175
    inherited BtnGerar: TCBitBtn
      Caption = '&Pesquisar'
    end
    inherited BtnFechar: TCBitBtn
      Left = 969
      ExplicitLeft = 969
    end
    object BtnImprime: TCBitBtn
      Left = 425
      Top = 4
      Width = 75
      Height = 25
      Caption = '&Imprimir'
      Glyph.Data = {
        
        E5FFFBFBFBFFFEFEFEFFFF00FF00FF00FF00FF00FF00FF00FF00}
      TabOrder = 3
      Visible = False
      OnClick = BtnImprimeClick
    end
  end
  object cxGrid: TcxGrid [2]
    Left = 0
    Top = 139
    Width = 1184
    Height = 423
    Hint = 'Clique com o bot'#227'o direito do mouse para exportar para Excel'
    Align = alBottom
    PopupMenu = PopupMenu
    TabOrder = 1
    object DBView: TcxGridDBTableView
      Navigator.Buttons.CustomButtons = <>
      DataController.DataSource = dsRelatorio
      DataController.KeyFieldNames = 'idCliente'
      DataController.Options = [dcoAssignGroupingValues, dcoAssignMasterDetailKeys, dcoSaveExpanding, dcoGroupsAlwaysExpanded]
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      OptionsBehavior.FocusCellOnTab = True
      OptionsData.CancelOnExit = False
      OptionsData.Deleting = False
      OptionsData.DeletingConfirmation = False
      OptionsData.Editing = False
      OptionsData.Inserting = False
      OptionsSelection.MultiSelect = True
      OptionsView.CellAutoHeight = True
      OptionsView.ColumnAutoWidth = True
      OptionsView.GroupFooters = gfVisibleWhenExpanded
      OptionsView.GroupSummaryLayout = gslAlignWithColumns
      OptionsView.Indicator = True
      object DBViewidCliente: TcxGridDBColumn
        Caption = 'Codigo'
        DataBinding.FieldName = 'idCliente'
        Width = 37
      end
      object DBViewidCadCliente: TcxGridDBColumn
        DataBinding.FieldName = 'idCadCliente'
        Visible = False
      end
      object DBViewdescCadCliente: TcxGridDBColumn
        Caption = 'Descri'#231#227'o'
        DataBinding.FieldName = 'descCadCliente'
        Width = 173
      end
      object DBViewdescAbreviada: TcxGridDBColumn
        Caption = 'Fantasia'
        DataBinding.FieldName = 'descAbreviada'
        Width = 106
      end
      object DBViewidCnpjCpf: TcxGridDBColumn
        Caption = 'CNPJ/CPF'
        DataBinding.FieldName = 'idCnpjCpf'
        Width = 108
      end
      object DBViewidInscEstadual: TcxGridDBColumn
        DataBinding.FieldName = 'idInscEstadual'
        Visible = False
      end
      object DBViewendereco: TcxGridDBColumn
        Caption = 'Endereco'
        DataBinding.FieldName = 'endereco'
        Width = 196
      end
      object DBViewnumero: TcxGridDBColumn
        Caption = 'N'#250'mero'
        DataBinding.FieldName = 'numero'
        Width = 52
      end
      object DBViewcomplemento: TcxGridDBColumn
        DataBinding.FieldName = 'complemento'
        Visible = False
      end
      object DBViewbairro: TcxGridDBColumn
        Caption = 'Bairro'
        DataBinding.FieldName = 'bairro'
        Width = 95
      end
      object DBViewcep: TcxGridDBColumn
        Caption = 'CEP'
        DataBinding.FieldName = 'cep'
        Width = 34
      end
      object DBViewidCidade: TcxGridDBColumn
        Caption = 'Cod. Cidade'
        DataBinding.FieldName = 'idCidade'
        Visible = False
        Width = 33
      end
      object DBViewdescCidade: TcxGridDBColumn
        Caption = 'Cidade'
        DataBinding.FieldName = 'descCidade'
        Width = 127
      end
      object DBViewidEstado: TcxGridDBColumn
        Caption = 'Estado'
        DataBinding.FieldName = 'idEstado'
        Width = 52
      end
      object DBViewfone: TcxGridDBColumn
        DataBinding.FieldName = 'fone'
        Visible = False
      end
      object DBViewfax: TcxGridDBColumn
        DataBinding.FieldName = 'fax'
        Visible = False
      end
      object DBViewcelular: TcxGridDBColumn
        DataBinding.FieldName = 'celular'
        Visible = False
      end
      object DBViewemail: TcxGridDBColumn
        Caption = 'Email'
        DataBinding.FieldName = 'email'
        Width = 157
      end
    end
    object Nivel1: TcxGridLevel
      GridView = DBView
      MaxDetailHeight = 200
    end
  end
  object CGroupBox1: TCGroupBox [3]
    Left = 0
    Top = 0
    Width = 1173
    Height = 100
    Caption = 'Dados para Filtragem'
    TabOrder = 0
    object CLabel36: TCLabel
      Left = 29
      Top = 19
      Width = 33
      Height = 13
      Alignment = taRightJustify
      Caption = 'Cidade'
    end
    object CLabel11: TCLabel
      Left = 546
      Top = 19
      Width = 16
      Height = 13
      Alignment = taRightJustify
      Caption = 'at'#233
    end
    object CLabel3: TCLabel
      Left = 385
      Top = 19
      Width = 70
      Height = 13
      Alignment = taRightJustify
      Caption = 'Data Cadastro'
    end
    object CLabel61: TCLabel
      Left = 653
      Top = 19
      Width = 29
      Height = 13
      Alignment = taRightJustify
      Caption = 'Grupo'
    end
    object CLabel62: TCLabel
      Left = 14
      Top = 44
      Width = 48
      Height = 13
      Alignment = taRightJustify
      Caption = 'Segmento'
    end
    object CLabel7: TCLabel
      Left = 136
      Top = 69
      Width = 16
      Height = 13
      Alignment = taRightJustify
      Caption = 'at'#233
    end
    object CLabel1: TCLabel
      Left = 16
      Top = 69
      Width = 46
      Height = 13
      Alignment = taRightJustify
      Caption = 'Cli/For de'
    end
    object DBEidCidade: TCDBEdit
      Left = 68
      Top = 16
      Width = 53
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'idCidade'
      DataSource = dsPadrao
      TabOrder = 0
      AcaoCad = frmPrincipal.actCadCidade
      DecimalControl = True
      KeyMode = kmNormal
    end
    object LookCidade: TCLookUp
      Left = 127
      Top = 16
      Width = 203
      Height = 19
      Alignment = taLeftJustify
      BevelInner = bvSpace
      BevelOuter = bvLowered
      Color = 14149350
      ParentBackground = False
      TabOrder = 1
      Key.Strings = (
        'idCidade')
      LookUpKey.Strings = (
        'idCidade')
      ClientDataSet = cdsLookUp
      DataSource = dsPadrao
      ValidaCampoObrigatorio = False
      CampoObrigatorio = True
      LookUpTable = 'Cidade'
      ReturnField = 'descCidade'
    end
    object LookEstado2: TCLookUp
      Left = 336
      Top = 16
      Width = 43
      Height = 19
      Alignment = taLeftJustify
      BevelInner = bvSpace
      BevelOuter = bvLowered
      Color = 14149350
      ParentBackground = False
      TabOrder = 2
      Key.Strings = (
        'idCidade')
      LookUpKey.Strings = (
        'idCidade')
      ClientDataSet = cdsLookUp
      DataSource = dsPadrao
      ValidaCampoObrigatorio = False
      CampoObrigatorio = True
      LookUpTable = 'Cidade'
      ReturnField = 'idEstado'
    end
    object DBEdtInicial: TCDBEdit
      Left = 461
      Top = 16
      Width = 79
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'dtInicial'
      DataSource = dsPadrao
      MaxLength = 10
      TabOrder = 3
      DecimalControl = True
      KeyMode = kmDate
    end
    object DBEdtFinal: TCDBEdit
      Left = 568
      Top = 16
      Width = 79
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'dtFinal'
      DataSource = dsPadrao
      MaxLength = 10
      TabOrder = 4
      DecimalControl = True
      KeyMode = kmDate
    end
    object DBEidGrupoCad: TCDBEdit
      Left = 688
      Top = 16
      Width = 53
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'idGrupoCad'
      DataSource = dsPadrao
      TabOrder = 5
      Find = dmFind.FindGrupoCad
      DecimalControl = True
      KeyMode = kmNormal
    end
    object LookGrupoCad: TCLookUp
      Left = 747
      Top = 16
      Width = 299
      Height = 19
      Alignment = taLeftJustify
      BevelInner = bvSpace
      BevelOuter = bvLowered
      Color = 14149350
      ParentBackground = False
      TabOrder = 6
      Key.Strings = (
        'idGrupoCad')
      LookUpKey.Strings = (
        'idGrupoCad')
      ClientDataSet = cdsLookUp
      DataSource = dsPadrao
      ValidaCampoObrigatorio = False
      CampoObrigatorio = True
      LookUpTable = 'GrupoCad'
      ReturnField = 'descGrupoCad'
    end
    object LookSegmento: TCLookUp
      Left = 127
      Top = 41
      Width = 297
      Height = 19
      Alignment = taLeftJustify
      BevelInner = bvSpace
      BevelOuter = bvLowered
      Color = 14149350
      ParentBackground = False
      TabOrder = 8
      Key.Strings = (
        'idSegmento')
      LookUpKey.Strings = (
        'idSegmento')
      ClientDataSet = cdsLookUp
      DataSource = dsPadrao
      ValidaCampoObrigatorio = True
      CampoObrigatorio = True
      LookUpTable = 'Segmento'
      ReturnField = 'descSegmento'
    end
    object DBEidSegmento: TCDBEdit
      Left = 68
      Top = 41
      Width = 53
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'idSegmento'
      DataSource = dsPadrao
      TabOrder = 7
      Find = dmFind.FindSegmento
      DecimalControl = True
      KeyMode = kmNormal
    end
    object DBRGOrdem: TCDBRadioGroup
      Left = 430
      Top = 41
      Width = 532
      Height = 50
      Caption = 'Ordenar por'
      Columns = 4
      DataField = 'ordem'
      DataSource = dsPadrao
      Items.Strings = (
        'Descri'#231#227'o'
        'Codigo'
        'Data de Cadastro'
        'Cidade')
      TabOrder = 9
      Values.Strings = (
        'D'
        'C'
        'A'
        'I')
    end
    object DBEidCadGeralFim: TCDBEdit
      Left = 158
      Top = 66
      Width = 62
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'idCadGeralFim'
      DataSource = dsPadrao
      TabOrder = 10
      DecimalControl = True
      KeyMode = kmNormal
    end
    object DBEidCadGeralIni: TCDBEdit
      Left = 68
      Top = 66
      Width = 62
      Height = 19
      BevelKind = bkTile
      BorderStyle = bsNone
      DataField = 'idCadGeralIni'
      DataSource = dsPadrao
      TabOrder = 11
      DecimalControl = True
      KeyMode = kmNormal
    end
  end
  inherited imgIcones: TImageList
    Bitmap = {
      
      000000000000}
  end
  inherited cdsRelatorio: TClientDataSet
    CommandText = 
      'SELECT Cad.idCadGeral idCliente, Adi.idCadAdicional idCadCliente' +
      ', Adi.descCadAdicional descCadCliente,  Adi.descAbreviada, Adi.i' +
      'dCnpjCpf, Adi.idInscEstadual,  Adi.endereco, Adi.numero,  Adi.co' +
      'mplemento, Adi.bairro, Adi.cep, Adi.idCidade, Cid.descCidade, ci' +
      'd.idEstado,  Adi.fone, Adi.fax, Adi.celular, Adi.email  FROM Cad' +
      'Geral Cad INNER JOIN CadAdicional Adi ON Cad.idCadGeral = Adi.id' +
      'CadGeral LEFT JOIN Cidade Cid ON Adi.idCidade = Cid.idCidade LEF' +
      'T JOIN CadGeralSegmento Seg ON Cad.idCadGeral = Seg.idCadGeral O' +
      'RDER BY Adi.descCadAdicional'
    Left = 549
    Top = 23
    object cdsRelatorioidCliente: TIntegerField
      FieldName = 'idCliente'
      Required = True
    end
    object cdsRelatorioidCadCliente: TSmallintField
      FieldName = 'idCadCliente'
      Required = True
    end
    object cdsRelatoriodescCadCliente: TStringField
      FieldName = 'descCadCliente'
      Required = True
      Size = 50
    end
    object cdsRelatoriodescAbreviada: TStringField
      FieldName = 'descAbreviada'
      Required = True
      Size = 30
    end
    object cdsRelatorioidCnpjCpf: TStringField
      FieldName = 'idCnpjCpf'
      Required = True
      Size = 18
    end
    object cdsRelatorioidInscEstadual: TStringField
      FieldName = 'idInscEstadual'
    end
    object cdsRelatorioendereco: TStringField
      FieldName = 'endereco'
      Required = True
      Size = 60
    end
    object cdsRelatorionumero: TIntegerField
      FieldName = 'numero'
    end
    object cdsRelatoriocomplemento: TStringField
      FieldName = 'complemento'
      Size = 30
    end
    object cdsRelatoriobairro: TStringField
      FieldName = 'bairro'
      Size = 30
    end
    object cdsRelatoriocep: TStringField
      FieldName = 'cep'
      Size = 10
    end
    object cdsRelatorioidCidade: TIntegerField
      FieldName = 'idCidade'
      Required = True
    end
    object cdsRelatoriodescCidade: TStringField
      FieldName = 'descCidade'
      Required = True
      Size = 40
    end
    object cdsRelatorioidEstado: TStringField
      FieldName = 'idEstado'
      Required = True
      Size = 2
    end
    object cdsRelatoriofone: TStringField
      FieldName = 'fone'
      Size = 18
    end
    object cdsRelatoriofax: TStringField
      FieldName = 'fax'
      Size = 18
    end
    object cdsRelatoriocelular: TStringField
      FieldName = 'celular'
      Size = 18
    end
    object cdsRelatorioemail: TStringField
      FieldName = 'email'
      Size = 60
    end
  end
  inherited cdsPadrao: TClientDataSet
    AfterInsert = cdsPadraoAfterInsert
    object cdsPadraoidCidade: TIntegerField
      FieldName = 'idCidade'
    end
    object cdsPadraoidSegmento: TIntegerField
      FieldName = 'idSegmento'
    end
    object cdsPadraoordem: TStringField
      FieldName = 'ordem'
      Size = 40
    end
    object cdsPadraoidCadGeralIni: TIntegerField
      FieldName = 'idCadGeralIni'
    end
    object cdsPadraoidCadGeralFim: TIntegerField
      FieldName = 'idCadGeralFim'
    end
    object cdsPadraodtInicial: TDateField
      FieldName = 'dtInicial'
    end
    object cdsPadraodtFinal: TDateField
      FieldName = 'dtFinal'
    end
    object cdsPadraoidGrupoCad: TIntegerField
      FieldName = 'idGrupoCad'
    end
  end
  object dsRelatorio: TDataSource
    DataSet = cdsRelatorio
    Left = 584
    Top = 24
  end
  object AbreArq: TOpenTextFileDialog
    DefaultExt = 'txt'
    Filter = 'Arquivos Texto (*.txt)|*.txt'
    Left = 736
    Top = 128
  end
  object AbrePasta: TcxShellBrowserDialog
    Left = 736
    Top = 192
  end
  object PopupMenu: TPopupMenu
    Images = imgIcones
    Left = 696
    Top = 128
    object actExportaExcel1: TMenuItem
      Caption = 'Exportar para Excel'
      ImageIndex = 2
      OnClick = actExportaExcel1Click
    end
  end
  object frxRel: TfrxReport
    Version = '5.2.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.MDIChild = True
    PreviewOptions.Modal = False
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.Author = 'Gesys Sistemas de Informa'#231#227'o'
    ReportOptions.CreateDate = 39263.419322395800000000
    ReportOptions.LastChange = 41234.360070416670000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      '    '
      'end.')
    Left = 488
    Top = 56
    Datasets = <
      item
        DataSet = frxDataset
        DataSetName = 'frxDataset'
      end
      item
        DataSet = dmPadrao.frxEmpresaContabil
        DataSetName = 'frxEmpresaContabil'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 256
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        Height = 143.093005800000000000
        Top = 18.897650000000000000
        Width = 1122.520410000000000000
        object Line2: TfrxLineView
          Left = 30.236240000000000000
          Top = 137.354360000000000000
          Width = 1058.268400000000000000
          Color = clBlack
          Frame.Color = clGray
          Frame.ShadowColor = clGray
          Diagonal = True
        end
        object lblEmpresa: TfrxMemoView
          Left = 445.866420000000000000
          Top = 15.322766300000000000
          Width = 419.527830000000000000
          Height = 37.795300000000000000
          AutoWidth = True
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -24
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxEmpresaContabil."descEmpresa"]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Picture1: TfrxPictureView
          Left = 26.456710000000000000
          Top = 15.204700000000000000
          Width = 155.000000000000000000
          Height = 62.000000000000000000
          DataField = 'imgLogo'
          DataSet = dmPadrao.frxEmpresaContabil
          DataSetName = 'frxEmpresaContabil'
          HightQuality = False
          Transparent = False
          TransparentColor = clWhite
        end
        object Memo7: TfrxMemoView
          Left = 940.559680000000000000
          Top = 12.338590000000000000
          Width = 143.622140000000000000
          Height = 117.165430000000000000
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -21
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'Listagem de Clientes e Fornecedores')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo30: TfrxMemoView
          Left = 445.984540000000000000
          Top = 53.118120000000010000
          Width = 419.527830000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            
              'CNPJ/CPF: [(<frxEmpresaContabil."idCnpjCpf">)]  IE: [(<frxEmpres' +
              'aContabil."idInscEstadual">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo31: TfrxMemoView
          Left = 445.984540000000000000
          Top = 72.795300000000000000
          Width = 419.527830000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            
              '[(<frxEmpresaContabil."endereco">)] - [(<frxEmpresaContabil."com' +
              'plemento">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo34: TfrxMemoView
          Left = 445.984540000000000000
          Top = 93.354360000000000000
          Width = 419.527830000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            
              'CEP: [(<frxEmpresaContabil."cep">)]  -  [(<frxEmpresaContabil."d' +
              'escCidade">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Memo35: TfrxMemoView
          Left = 445.984540000000000000
          Top = 113.031540000000000000
          Width = 419.527830000000000000
          Height = 18.897650000000000000
          AutoWidth = True
          DataSet = FrmPadraoRel.frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'Fone: [(<frxEmpresaContabil."fones">)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaCenter
        end
        object Line4: TfrxLineView
          Left = 912.102970000000000000
          Top = 11.425170000000000000
          Height = 120.944960000000000000
          Color = clBlack
          Frame.Color = clGray
          Frame.ShadowColor = clGray
          Frame.Typ = [ftLeft]
        end
      end
      object GHRelatorio: TfrxGroupHeader
        FillType = ftBrush
        Height = 24.897650000000000000
        Top = 222.992270000000000000
        Width = 1122.520410000000000000
        Condition = 'frxDataset."idEmpresa"'
        ExpandDrillDown = True
        ReprintOnNewPage = True
        StartNewPage = True
        object Shape1: TfrxShapeView
          Left = 26.456692910000000000
          Top = 4.133890000000008000
          Width = 1084.724636460000000000
          Height = 15.118120000000000000
          Fill.BackColor = 15724527
          Frame.Color = 15790320
        end
        object DBMTitidGrupo: TfrxMemoView
          Left = 30.236222910000000000
          Top = 3.779527559999991000
          Width = 37.795300000000000000
          Height = 15.118110240000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'C'#243'digo')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo36: TfrxMemoView
          Left = 78.433210000000000000
          Top = 3.779527559999991000
          Width = 49.834570000000000000
          Height = 15.118107800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Descri'#231#227'o')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo10: TfrxMemoView
          Left = 661.315400000000000000
          Top = 3.779527559999991000
          Width = 42.275510000000000000
          Height = 15.118107800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'N'#250'mero')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo1: TfrxMemoView
          Left = 538.149970000000100000
          Top = 3.779527559999991000
          Width = 53.614100000000000000
          Height = 15.118107800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Endere'#231'o')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo3: TfrxMemoView
          Left = 283.464750000000000000
          Top = 3.779529999999994000
          Width = 42.275510000000000000
          Height = 15.118107800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Fantasia')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo5: TfrxMemoView
          Left = 429.984540000000000000
          Top = 3.779529999999994000
          Width = 53.614100000000000000
          Height = 15.118107800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'CNPJ/CPF')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo9: TfrxMemoView
          Left = 716.567410000000000000
          Top = 3.779529999999994000
          Width = 42.275510000000000000
          Height = 15.118107800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Bairro')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo14: TfrxMemoView
          Left = 838.732840000000000000
          Top = 3.779529999999994000
          Width = 42.275510000000000000
          Height = 15.118107800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'CEP')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo18: TfrxMemoView
          Left = 899.882500000000000000
          Top = 3.779529999999994000
          Width = 42.275510000000000000
          Height = 15.118107800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Cidade')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo20: TfrxMemoView
          Left = 1039.370750000000000000
          Top = 3.779529999999994000
          Width = 30.936920000000000000
          Height = 15.118107800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Email')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo22: TfrxMemoView
          Left = 1002.016390000000000000
          Top = 3.779529999999994000
          Width = 34.716449999999990000
          Height = 15.118107800000000000
          AutoWidth = True
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Estado')
          ParentFont = False
          VAlign = vaBottom
        end
      end
      object Band4: TfrxMasterData
        FillType = ftBrush
        Height = 18.141754250000000000
        Top = 272.126160000000000000
        Width = 1122.520410000000000000
        Columns = 1
        ColumnWidth = 200.000000000000000000
        ColumnGap = 20.000000000000000000
        DataSet = frxDataset
        DataSetName = 'frxDataset'
        RowCount = 0
        object DBMidGrupo: TfrxMemoView
          Left = 25.236240000000000000
          Top = 0.755905509999990900
          Width = 47.826840000000000000
          Height = 15.118110240000000000
          DataField = 'idCliente'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDataset."idCliente"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo8: TfrxMemoView
          Left = 81.330860000000000000
          Top = 0.755905509999990900
          Width = 195.228510000000000000
          Height = 15.118110240000000000
          StretchMode = smMaxHeight
          DataField = 'descCadCliente'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.4n'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDataset."descCadCliente"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo11: TfrxMemoView
          Left = 662.551640000000000000
          Top = 0.755905509999990900
          Width = 40.267780000000000000
          Height = 15.118110240000000000
          StretchMode = smMaxHeight
          DataField = 'numero'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDataset."numero"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo2: TfrxMemoView
          Left = 539.709030000000000000
          Top = 0.755905509999990900
          Width = 115.858380000000000000
          Height = 15.118110240000000000
          StretchMode = smMaxHeight
          DataField = 'endereco'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDataset."endereco"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo4: TfrxMemoView
          Left = 286.362400000000000000
          Top = 0.755907949999993900
          Width = 142.315090000000000000
          Height = 15.118110240000000000
          StretchMode = smMaxHeight
          DataField = 'descAbreviada'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.4n'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDataset."descAbreviada"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo6: TfrxMemoView
          Left = 432.882190000000000000
          Top = 0.755907949999993900
          Width = 100.740260000000000000
          Height = 15.118110240000000000
          StretchMode = smMaxHeight
          DataField = 'descCadCliente'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.4n'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDataset."descCadCliente"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo12: TfrxMemoView
          Left = 717.803650000000000000
          Top = 0.755907949999993900
          Width = 115.858380000000000000
          Height = 15.118110240000000000
          StretchMode = smMaxHeight
          DataField = 'bairro'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDataset."bairro"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo16: TfrxMemoView
          Left = 839.969080000000000000
          Top = 0.755907949999993900
          Width = 55.385900000000000000
          Height = 15.118110240000000000
          StretchMode = smMaxHeight
          DataField = 'cep'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDataset."cep"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo19: TfrxMemoView
          Left = 898.118740000000000000
          Top = 0.755907949999993900
          Width = 108.299320000000000000
          Height = 15.118110240000000000
          StretchMode = smMaxHeight
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descCidade">)]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo21: TfrxMemoView
          Left = 1037.606990000000000000
          Top = 0.755907949999993900
          Width = 78.063080000000000000
          Height = 15.118110240000000000
          StretchMode = smMaxHeight
          DataField = 'email'
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[frxDataset."email"]')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo23: TfrxMemoView
          Left = 1009.354980000000000000
          Width = 21.370130000000000000
          Height = 15.118110240000000000
          DataSet = frxDataset
          DataSetName = 'frxDataset'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<frxDataset."descCidade">)]')
          ParentFont = False
          VAlign = vaBottom
        end
      end
      object GroupFooter1: TfrxGroupFooter
        FillType = ftBrush
        Height = 24.456710000000000000
        Top = 313.700990000000000000
        Width = 1122.520410000000000000
      end
      object PageFooter1: TfrxPageFooter
        FillType = ftBrush
        Height = 66.811070000000000000
        Top = 400.630180000000000000
        Width = 1122.520410000000000000
        object Memo13: TfrxMemoView
          Left = 316.157700000000000000
          Top = 7.181102362204740000
          Width = 117.165430000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Nilus Report ')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo15: TfrxMemoView
          Left = 26.456692910000000000
          Top = 7.181102362204740000
          Width = 275.905690000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Nilus Solu'#231#245'es para o Agroneg'#243'cio Ltda.')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Memo17: TfrxMemoView
          Left = 618.583180000000000000
          Top = 7.181102362204740000
          Width = 132.283550000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[(<Page>)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
        object Line1: TfrxLineView
          Left = 26.456692910000000000
          Top = 3.401359999999898000
          Width = 733.228346460000000000
          Color = clBlack
          Frame.Typ = [ftTop]
          Frame.Width = 0.100000000000000000
        end
        object Memo58: TfrxMemoView
          Left = 454.000000000000000000
          Top = 7.181102359999898000
          Width = 117.165430000000000000
          Height = 11.338590000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[(<Date>)] - [(<Time>)]')
          ParentFont = False
          WordWrap = False
          VAlign = vaBottom
        end
      end
    end
  end
  object dspLookUp: TDataSetProvider
    DataSet = dmPadrao.sdsLookup
    Options = [poPropogateChanges, poAllowCommandText]
    Left = 456
    Top = 184
  end
  object cdsLookUp: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspLookUp'
    Left = 488
    Top = 184
  end
end
