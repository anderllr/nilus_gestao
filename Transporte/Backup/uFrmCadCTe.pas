...
uses
  ...
  ACBrCTeDACTEClass, ACBrCTeDACTEFR;
...
begin
  ...
  begin
    with frmPrincipal.ExecutaSQLRet([DBEidEmpresa], '', 'ConfigTransporte') do
    begin
...
begin
  ...
  if cdsPadrao.FieldByName('toma').AsString = '4' then
    if trim(DBEidTomador.Text) = '' then
  ...
  try
    ...
    GravaChave(2, DBEidDocSerie.Text, DBEidCte);
    inherited;
...
begin
  ...
  begin
    ...
    caminho := ACBrCTe1.Configuracoes.Geral.PathSalvar + cdsCTeEledescXML.AsString;
...
begin
  msg := '';
  try
    sql := 'SELECT * FROM ConfigTransporte WHERE idEmpresa = ' + DBEidEmpresa.Text;
    ...
    begin
      ...
      begin
        ...
        if not (DirectoryExists(caminho)) then
          CreateDir(caminho);
        ACBrCTe1.Configuracoes.Geral.PathSalvar := caminho;
        ACBrCTe1.Configuracoes.Arquivos.PathCan := caminho;
        ...
        ACBrCTe1.Configuracoes.Arquivos.PathCCe := caminhocce;
...
begin
  if trim(DBEidDocSerie.Text) = '' then
    exit;
  inherited;
  if (not (BuscaChave(cdsPadrao, chave))) and (trim(DBEidCTe.Text) <> '') then
  begin
...
begin
  ...
  funcao.SomenteLeitura([DBEtpServ, DBEdtEmissao, DBEtpCTe, DBEmodal, DBEforPag, DBEidNatureza,
                         DBErefCTe, DBEidDestinatario, DBEidCadDestinatario, DBEidRemetente,
                         DBEidExpedidor, DBEidRecebedor, DBRGtoma, DBEidTomador, DBEidSeguradora,
                         ...
                         DBEvlICMSST, DBEvlNF, DBEvlProduto, DBEqtPeso, DBEdtPrevisaNF,
...
begin
  funcao.SomenteLeitura([DBEidTomador], cdsPadrao.FieldByName('toma').AsString <> '4');
end;
