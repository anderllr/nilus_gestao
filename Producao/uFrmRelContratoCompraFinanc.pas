unit uFrmRelContratoCompraFinanc;
...
type
  TFrmRelContratoCompraFinanc = class(TFrmPadraoRelEmpresa)
    FrxRel: TfrxReport;
    CLabel4: TCLabel;
    DBEidSafra: TCDBEdit;
    LookSafra: TCLookUp;
...
var
  FrmRelContratoCompraFinanc: TFrmRelContratoCompraFinanc;
...
procedure TFrmRelContratoCompraFinanc.actGerarExecute(Sender: TObject);
var
...
//  inherited;
  ...
  sql := 'EXEC spRelContratoCompraFinanc ' + funcao.RetornaValorEField(DBEidEmpresa, 3) + ', '
  ...
  (frxRel.FindComponent('MPAGARZERO') as TfrxMemoview).clear;
  ...
  if DBCKmostraAPagarZero.Checked then
    (frxRel.FindComponent('MPAGARZERO') as TfrxMemoview).Text := 'X';
...
procedure TFrmRelContratoCompraFinanc.cdsPadraoAfterInsert(DataSet: TDataSet);
begin
  inherited;
  ...
  cdsPadraomostraAPagarZero.AsString := 'S';
  cdsPadraoAFixar.AsString := 'N';
end;

procedure TFrmRelContratoCompraFinanc.cdsRelatorioAfterInsert(DataSet: TDataSet);
begin
  inherited;
  DataSet.FieldByName('mostrarZero').AsString := 'S';
end;

procedure TFrmRelContratoCompraFinanc.DBEidContratoKeyDown(Sender: TObject;
...
procedure TFrmRelContratoCompraFinanc.DBEidEmbarqueKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = VK_F4 then
...
procedure TFrmRelContratoCompraFinanc.DBEidEmbarqueKeyPress(Sender: TObject;
  var Key: Char);
var
  pRect:TRect;
begin
...
procedure TFrmRelContratoCompraFinanc.DBEidFornecedorKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = VK_F4 then
...
procedure TFrmRelContratoCompraFinanc.DBEidFornecedorKeyPress(Sender: TObject;
  var Key: Char);
var
  pRect:TRect;
begin
...
procedure TFrmRelContratoCompraFinanc.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  FrmRelContratoCompraFinanc := nil;
end;

procedure TFrmRelContratoCompraFinanc.FormCreate(Sender: TObject);
begin
  inherited;
  fiscal := True;
  relatorio := 'RelContratoCompraFinanc.fr3';
  par := 0;
  Height := 360;
  Width :=  540;
end;

procedure TFrmRelContratoCompraFinanc.FrxRelAfterPrint(Sender: TfrxReportComponent);
begin
  inherited;
