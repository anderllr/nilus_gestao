...
uses
  ...
  CDBRadioGroup, CDBCheckBox, System.Actions;

type
  TFrmFechaOrdemFrete = class(TFrmPadrao)
    ...
    cdsEmbarqueqtEmbarque: TIntegerField;
    cdsEmbarqueqtSaldo: TIntegerField;
...
var
  FrmFechaOrdemFrete: TFrmFechaOrdemFrete;

implementation

uses uDmPadrao, uDmFind, uFrmGerenciaRequisicao, uFrmPrincipal, uFuncao,
  uFrmPesCotacao, uFrmPesEmpFiscal, uFrmNotaFiscal;

{$R *.dfm}

procedure TFrmFechaOrdemFrete.AbreCli(codigo: integer);
begin
...
procedure TFrmFechaOrdemFrete.AbreEmbarqueCli(codigo: integer);
begin
...
procedure TFrmFechaOrdemFrete.BtnConsultarClick(Sender: TObject);
begin
  inherited;
  AbreCli(cdsClienteidCliente.AsInteger);
end;

procedure TFrmFechaOrdemFrete.BtnRetirarClick(Sender: TObject);
var
  sql : string;
begin
  inherited;
...
procedure TFrmFechaOrdemFrete.BtnSalvarClick(Sender: TObject);
var
  sql : string;
...
procedure TFrmFechaOrdemFrete.cdsEmbarqueAfterOpen(DataSet: TDataSet);
var
  i : integer;
begin
  inherited;
...
procedure TFrmFechaOrdemFrete.cdsPadraoAfterInsert(DataSet: TDataSet);
begin
  inherited;
  cdsPadraoidFrete.AsInteger := 0;
end;

procedure TFrmFechaOrdemFrete.DBEcepExit(Sender: TObject);
begin
  inherited;
...
procedure TFrmFechaOrdemFrete.DBEidCnpjCpfEnter(Sender: TObject);
begin
  inherited;
  vlAnterior := (Sender as TCustomEdit).Text;
end;

procedure TFrmFechaOrdemFrete.DBEidCnpjCpfExit(Sender: TObject);
var
  cpf : string;
begin
  inherited;
...
procedure TFrmFechaOrdemFrete.DBEidDocSerieExit(Sender: TObject);
var
  sql : string;
begin
  inherited;
  ...
  begin
    ...
    if (trim((Sender as TCustomEdit).Text) <> '') then
    begin
      sql := 'SELECT MAX(idNf) idNf FROM Nf WHERE idEmpresa = ' + DBEidEmpresa.Text + ' AND idCadEmpresa = ' +
             DBEidCadEmpresa.Text + ' AND idDocSerie = ' + QuotedStr(DBEidDocSerie.Text);
...
procedure TFrmFechaOrdemFrete.DBEidEmpresaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  if Key = VK_F4 then
...
procedure TFrmFechaOrdemFrete.DBEidNfExit(Sender: TObject);
begin
  inherited;
...
procedure TFrmFechaOrdemFrete.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
...
procedure TFrmFechaOrdemFrete.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TFrmFechaOrdemFrete.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TFrmFechaOrdemFrete.GridCotacaoFornecedorCellClick(Column: TColumn);
begin
  inherited;
  AbreEmbarqueCli(cdsEmbarqueCliidCliente.AsInteger);
end;

procedure TFrmFechaOrdemFrete.HabilitaMotorista(hab: boolean);
begin
  funcao.SomenteLeitura([DBEdescCadAdicional, DBEidCidade, DBEcep, DBEbairro, DBEfone, DBEendereco, DBEnumero], not hab);
end;

procedure TFrmFechaOrdemFrete.LimpaMotorista;
begin
  if not (cdsPadrao.State in [dsEdit, dsInsert]) then
    cdsPadrao.Edit;
