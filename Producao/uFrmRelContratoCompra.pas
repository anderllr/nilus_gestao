...
type
  TFrmRelContratoCompra = class(TFrmPadraoRelEmpresa)
    FrxRel: TfrxReport;
    CLabel4: TCLabel;
    DBEidSafra: TCDBEdit;
    LookSafra: TCLookUp;
...
var
  FrmRelContratoCompra: TFrmRelContratoCompra;
...
procedure TFrmRelContratoCompra.actGerarExecute(Sender: TObject);
var
...
//  inherited;
  ...
  sql := 'EXEC spRelContratoCompra ' + funcao.RetornaValorEField(DBEidEmpresa, 3) + ', '
...
procedure TFrmRelContratoCompra.cdsPadraoAfterInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure TFrmRelContratoCompra.cdsRelatorioAfterInsert(DataSet: TDataSet);
begin
  inherited;
  DataSet.FieldByName('mostrarZero').AsString := 'S';
end;

procedure TFrmRelContratoCompra.DBEidContratoKeyDown(Sender: TObject;
...
procedure TFrmRelContratoCompra.DBEidEmbarqueKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = VK_F4 then
...
procedure TFrmRelContratoCompra.DBEidEmbarqueKeyPress(Sender: TObject;
  var Key: Char);
var
  pRect:TRect;
begin
...
procedure TFrmRelContratoCompra.DBEidFornecedorKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = VK_F4 then
...
procedure TFrmRelContratoCompra.DBEidFornecedorKeyPress(Sender: TObject;
  var Key: Char);
var
  pRect:TRect;
begin
...
procedure TFrmRelContratoCompra.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  FrmRelContratoCompra := nil;
end;

procedure TFrmRelContratoCompra.FormCreate(Sender: TObject);
begin
  inherited;
  fiscal := True;
  relatorio := 'RelContratoCompra.fr3';
  par := 0;
  Height := 340;
  Width :=  540;
end;

procedure TFrmRelContratoCompra.FrxRelAfterPrint(Sender: TfrxReportComponent);
begin
  inherited;
